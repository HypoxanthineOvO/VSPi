...
id: decision-d3cdb7ff76556bda0c4812e50be39d24
kind: decision
schema_version: '1'
scope:
  ref: vspi-terminal-mock-recovery
  type: cycle
semantic_hash: d3cdb7ff76556bda0c4812e50be39d24a705b1cdcdda47a27f1e91d83e1e6463
source_refs:
  - locator: compiled-plan
    ref: cycle:vspi-terminal-mock-recovery:revision:4
    type: delivery_plan
supersedes:
  - decision-3ce0c2c33314cdab7fe0fae3c65c8c85
updated_at: 2026-08-01T13:08:33.545Z
---
# VSPi 独立方框 Question 选项与语义滚动 Mock

在保持已验证的输入区、背景中性主题、宽松 Resume、增量发送与原子 hydration 前提下，将每个 Question 选项渲染为具有完整四边的独立方框，并以语义行范围保证多行选中框整体滚动；经用户按 Frame 和行号验收后再处理生产集成，快捷键 footer 暂留原位等待具体反馈。
...
```json
{
  "acceptance": {
    "criteria": [
      ...
      },
      {
        ...
        "statement": "Question 元信息、题目、选项实体、输入区和操作提示按视觉分组留出稳定间距；每个选项拥有独立完整的方形边框，相邻方框之间保留一整行空白，不共享边线。",
        "verification": "Question snapshot 与 80×40 PTY trace 断言每项均有上、左、右、下边界及框间空行；长说明保持在同一框内，输入区与外围 Status 坐标不变。"
      },
      {
      ...
      },
      {
        ...
        "statement": "单选使用 (●)/( )，多选使用 [✓]/[ ]，排序使用稳定序号；选中项由 ›、bold/focus 文字及强调边框共同表达，不使用整行 inverse、白色背景或固定背景色。",
        "verification": "Panel snapshot 覆盖 singleChoice、multiChoice、ranking、other、长说明、窄宽、无色、Terminal 与 Light；断言独立方框四边、控件宽度、框间空行、无 inverse/背景填充且状态不只依赖颜色。"
      },
      {
      ...
      {
        "id": "question-box-scroll",
        "statement": "Question 选项渲染器产出选中方框的语义起止行，内部滚动以完整方框范围定位，不再通过导轨或选项文本猜测块边界。",
        "verification": "短高、长 label、长 description 与多选项测试移动到首项和末项，断言能容纳时完整选中框可见；超高单框保持框顶可见且 footer 固定。"
      }
    ],
    "scope": "plan"
  },
  ...
  "constraints": [
    ...
    "每个选项使用独立方形 Unicode 边框，no-color/ASCII 环境退化为 +、-、|；不得复用 Question 外框的圆角符号制造假圆角。",
    "相邻选项框之间保留一整行空白；多行 label 与 description 必须留在同一框内。",
  ...
  "milestones": [
    {
      ...
      "order": 1,
      "outcome": "保持输入区、Terminal/Light、Resume、通知与性能回归通过，使每个 Question 选项以独立完整方框呈现，并让多行选中框按语义范围滚动；footer 原位保留供人工继续观察。",
      "stone": {
        "acceptance_criteria": [
          "每个选项都有独立完整方形边框，不再只是相邻横线分段。",
          "相邻方框之间有一整行空白，边界不共享；多行说明位于对应框内。",
          "单选 (●)/( )、多选 [✓]/[ ]、排序与其他状态清楚，选中框没有白色/反色背景。",
          "长选项与短终端滚动时，能容纳的选中方框整体可见，footer 保持固定。",
          "Question 输入区保持 revision 3 的标签、› 输入行与底部横线，不重新添加导轨。",
          "Terminal 黑底/白底、no-color 与显式 Light 均可辨，Resume 会话间距不回退。",
          "通知、发送、长输出、Resume hydration 与恢复落点保持此前正确方向。",
          "用户可继续用 Frame/行号描述 footer 位置问题，footer 未经具体反馈不作为本轮已解决项。",
          "用户明确允许进入生产构建与本地分发。"
        ],
        "id": "stone-terminal-mock-review",
        "review": "用户分别运行 terminal 与 light 的 80×40 Mock，检查每个选项的独立完整方框、框间空行、长说明归属、较大控件、无白块焦点和滚动；输入区应保持当前方案，快捷键 footer 只记录具体 Frame/行号反馈。"
      },
      "title": "独立方框 Question 选项与语义滚动 Mock",
      "verification_criteria": [
        ...
        "每个 single、multi、ranking、other 选项均有独立上/左/右/下方形边界，框间一整行空白且不共享边线。",
        "长 label 与 description 在同一方框内完整换行；选中框起止行由渲染器显式提供给滚动逻辑。",
        "选中框使用 ›、较大控件、文字和边框强调，无 inverse、白色背景或固定背景色。",
        "输入区、footer、Question 外框、空白 gutter、两行 Status、通知与 Resume 维持 revision 3 几何。",
        "Terminal/Light/no-color、短终端、长选项、首末项滚动与历史 ANSI Frame 均有断言。",
  ...
  "outcome": "在保持已验证的输入区、背景中性主题、宽松 Resume、增量发送与原子 hydration 前提下，将每个 Question 选项渲染为具有完整四边的独立方框，并以语义行范围保证多行选中框整体滚动；经用户按 Frame 和行号验收后再处理生产集成，快捷键 footer 暂留原位等待具体反馈。",
  "revision": 4,
  "schema_version": "1",
  "status": "draft",
  "title": "VSPi 独立方框 Question 选项与语义滚动 Mock",
  "plan_hash": "0bb7b03699de9e83ecccd6ea65d768b825a34fc41450bd0d82393dd3cc89b400"
}
```
