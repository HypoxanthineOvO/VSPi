...
id: decision-a9f8f997782c21e59dcbdc7ffde3b766
kind: decision
schema_version: '1'
scope:
  ref: vspi-terminal-mock-recovery
  type: cycle
semantic_hash: a9f8f997782c21e59dcbdc7ffde3b76654f44ee3f39120613daba82fac6c57b0
source_refs:
  - locator: compiled-plan
    ref: cycle:vspi-terminal-mock-recovery:revision:5
    type: delivery_plan
supersedes:
  - decision-d3cdb7ff76556bda0c4812e50be39d24
updated_at: 2026-08-01T13:30:11.023Z
---
# VSPi 无间隔紧凑 Question 选项列表 Mock

在保持已验证的输入区、背景中性主题、宽松 Resume、短终端标题固定、增量发送与原子 hydration 前提下，将 Question 选项收敛为无空行、无分隔线、无内层方框的紧凑列表，只保留焦点与题型状态标记；经用户按 Frame 和行号验收后再处理生产集成。
...
```json
{
  ...
  "milestones": [
    {
      ...
      "order": 1,
      "outcome": "保持输入区、Terminal/Light、Resume、通知、短终端标题与性能回归通过，使 Question 选项以无空行、无分隔、无方框的连续列表呈现，仅保留题型与焦点标记。",
      "stone": {
        "acceptance_criteria": [
          "普通选项连续逐行排列，中间没有空行。",
          "选项区域没有横向分隔线、左右导轨或任何内层方框。",
          "单选 (●)/( )、多选 [✓]/[ ]、排序、其他与 › 焦点标记清楚。",
          "选中项没有白色/反色背景；Terminal 黑底/白底、no-color 与显式 Light 均可辨。",
          "长 label/description 只在必要时紧邻换行，短终端保留标题与完整选中项。",
          "Question 输入区保持标签、› 输入行与底部横线；footer 保持当前位置。",
          "Resume 会话间距、通知、发送、长输出、hydration 与恢复落点不回退。",
          "用户明确允许进入生产构建与本地分发。"
        ],
        "id": "stone-terminal-mock-review",
        "review": "用户分别运行 terminal 与 light 的 80×40 Mock，检查普通选项连续无间隔、无横线/导轨/方框，前置标记清楚，长说明必要时紧邻换行；输入区、footer、白底与 Resume 不应回退。"
      },
      "title": "无间隔紧凑 Question 选项列表 Mock",
      "verification_criteria": [
        ...
        "single、multi、ranking、other 普通短选项逐行连续排列，选项间无空白、横线、导轨或内层方框。",
        "长 label 与 description 仅在宽度不足时紧邻换行；选中项起止行显式提供给滚动逻辑。",
        "选中项使用 ›、较大控件及文字强调，无 inverse、白色背景或固定背景色。",
        "输入区、footer、Question 外框、空白 gutter、两行 Status、通知与 Resume 维持已认可几何。",
        "短终端固定标题与完整选中项、Terminal/Light/no-color、首末项滚动与历史 ANSI Frame 均有断言。",
  ...
  "outcome": "在保持已验证的输入区、背景中性主题、宽松 Resume、短终端标题固定、增量发送与原子 hydration 前提下，将 Question 选项收敛为无空行、无分隔线、无内层方框的紧凑列表，只保留焦点与题型状态标记；经用户按 Frame 和行号验收后再处理生产集成。",
  "revision": 5,
  "schema_version": "1",
  "status": "draft",
  "title": "VSPi 无间隔紧凑 Question 选项列表 Mock",
  "plan_hash": "19fa49a0a0822de7076c1afdbb881508088c38b4b476e594e602ae7adee59f18"
}
```
