...
id: decision-b8914dccf4f709762381abeb08e0d370
kind: decision
schema_version: '1'
scope:
  ref: vspi-v0-2-0-workflow-integration
  type: cycle
semantic_hash: b8914dccf4f709762381abeb08e0d370c9b0a260c7457b48a3f6b60e5aef5d2d
source_refs:
  - locator: compiled-plan
    ref: cycle:vspi-v0-2-0-workflow-integration:revision:5
    type: delivery_plan
supersedes:
  - decision-5a7bb7573abaa22b1e23d58c51980dad
updated_at: 2026-07-25T03:58:33.835Z
---
# VSPi v0.2.0 可用性优先与 Workflow 集成

交付可靠、可中断、按事件瀑布展示的原生工具体验，并完成 Workflow 单一权威、Session 隔离及扩展工具边界。 修复跨后端默认模型污染，并以真实 DeepSeek PTY 会话作为最终验收门槛。
...
```json
{
  "acceptance": {
    "criteria": [
      ...
      {
        "id": "runtime-default-isolation",
        "statement": "Fixture 与 Pi 的模型默认值不会互相污染，失效默认值不能阻断正常启动。",
        "verification": "覆盖 Fixture 保存、污染文件兼容、有效 Pi 默认恢复和真实 npm run dev 启动。"
      },
      {
        "id": "deepseek-pty",
        "statement": "最终交付必须经过真实 DeepSeek 终端会话与视觉瀑布检查。",
        "verification": "真实 PTY 选择 DeepSeek，提交工具任务，抓取并审查终端输出、工具链顺序、最终回答、Esc 与关键面板。"
      }
    ],
    "scope": "plan"
  },
  ...
  "constraints": [
    ...
    "只允许为最终 PTY 验证调用用户明确授权的 DeepSeek；不调用其他付费远程模型。",
    "已污染的 runtime-defaults.json 必须向后兼容，不要求用户手工删除配置。"
  ],
  "delivery_kind": "cycle",
  "delivery_mode": "plan",
  "evidence": [
    ...
    {
      "ref": "conversation-2026-07-25-runtime-default-crash-and-deepseek-pty",
      "summary": "用户报告正常 Pi 启动被 fixture/offline-fixture 默认值阻断，并要求真实 DeepSeek PTY 全流程与终端式汇报。",
      "type": "user-feedback"
    }
  ],
  ...
  "milestones": [
    ...
    },
    {
      ...
      "verification_criteria": [
        ...
        "Fixture 不持久化离线模型身份；Pi 遇到跨后端或失效默认模型时保留当前可用模型并显示有界警告，不中止启动。",
        "真实 PTY 选择 DeepSeek，完成至少一轮包含工具调用的输入、瀑布输出、最终回复与视觉检查，并保存去 ANSI 的终端摘录作为证据。"
      ]
    }
  ],
  "outcome": "交付可靠、可中断、按事件瀑布展示的原生工具体验，并完成 Workflow 单一权威、Session 隔离及扩展工具边界。 修复跨后端默认模型污染，并以真实 DeepSeek PTY 会话作为最终验收门槛。",
  "revision": 5,
  "schema_version": "1",
  "status": "draft",
  "title": "VSPi v0.2.0 可用性优先与 Workflow 集成",
  "plan_hash": "27283229081b82a95747300b64aca68739e9d3919775919990503f353dde0028"
}
```
