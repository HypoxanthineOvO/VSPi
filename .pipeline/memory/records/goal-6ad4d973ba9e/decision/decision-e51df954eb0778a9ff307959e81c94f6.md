...
id: decision-e51df954eb0778a9ff307959e81c94f6
kind: decision
schema_version: '1'
scope:
  ref: vspi-persistent-goal-runner
  type: goal
semantic_hash: e51df954eb0778a9ff307959e81c94f6df305645b0d5492d967dcd5786da81c9
source_refs:
  - locator: compiled-plan
    ref: goal:vspi-persistent-goal-runner:revision:2
    type: delivery_plan
supersedes:
  - decision-347778f17c1ed58fcea811fd2175c413
updated_at: 2026-08-01T05:46:19.540Z
---
# VSPi 持久 Goal Runner与终端瀑布修订

VSPi 通过 `/goal` 提供项目持久、可恢复且有安全边界的长期任务执行：模型维护 Working Plan 与 Progress Markers，普通阶段总结不会终止未完成 Goal，只有真实完成、阻塞、暂停、取消或预算边界能够停止自动续跑。 同时修正启动与 Transcript 的物理终端布局，使 Splash、内容、Composer 和状态行按真实瀑布连续增长与滚动。 Question 交互区与主 Composer 之间保留稳定视觉间隔，避免等待态的双输入区域拥挤。
...
```json
{
  ...
  "design": {
    ...
    "outcome": "VSPi 通过 `/goal` 提供项目持久、可恢复且有安全边界的长期任务执行：模型维护 Working Plan 与 Progress Markers，普通阶段总结不会终止未完成 Goal，只有真实完成、阻塞、暂停、取消或预算边界能够停止自动续跑。 同时修正启动与 Transcript 的物理终端布局，使 Splash、内容、Composer 和状态行按真实瀑布连续增长与滚动。 Question 交互区与主 Composer 之间保留稳定视觉间隔，避免等待态的双输入区域拥挤。"
  },
  "evidence": [
    {
      "ref": "current-chat-2026-07-31",
  ...
  "plan_hash": "ae8ab808c574a8adbc480c4352c2cdd92b5068c7a12c09212e770c65b50625fa"
}
```
