...
首次确认在线提交时，客户端按设备名-用户名自动登记并保存私有身份缓存，无需管理员发文件或填写模型 API Key。预览显示归属标识；只导出、预览或取消时不登记。需要接收服务已支持自动登记；网络或服务不可用时，本地包保留供稍后重试。旧 `feedback.json` 仍兼容，但不再是使用条件。不要分享身份缓存；未知秘密仍可能漏检，必须检查具体内容。
