...
describe('Agent tools', () => {
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(ctx.allEvents.findLast((event) => event.event === 'toolCall')?.args).toMatchObject({ toolCallId: 'call_lookup', args: { query: 'moon' } });
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        system: <system-prompt>
      ...
      await ctx.untilTurnEnd();
      expect(ctx.allEvents.findLast((event) => event.type === '[rpc]' && event.event === 'turn.ended')?.args).toMatchObject({ reason: 'completed' });
      expect(ctx.allEvents.find((event) => event.type === '[rpc]' && event.event === 'tool.result')?.args).toMatchObject({ toolCallId: 'call_lookup', output: 'moon-result' });
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
      messages:
        <last>
