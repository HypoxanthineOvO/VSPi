...
- Subagents default to a 2-hour timeout, configurable through `[subagent].timeout_ms` or `KIMI_SUBAGENT_TIMEOUT_MS`. If one times out, resume the same agent instead of starting over.
