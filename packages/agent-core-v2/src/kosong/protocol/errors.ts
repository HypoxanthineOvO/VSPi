...
export const ProtocolErrors = {
  ...
  info: {
    ...
    'provider.protocol_error': {
      ...
      retryable: false,
      public: true,
      action: 'Check the effective model protocol, endpoint and gateway response format.',
    },
    'provider.incomplete_stream': {
      title: 'Provider stream ended before completion',
      retryable: true,
      public: true,
      action: 'Retry within the recovery budget; check upstream termination if the failure repeats.',
    },
    'provider.rate_limit': {
