...
export function getActionDefinition(command: CommandDefinition | string): ActionDefinition | undefined {
...
export function matchCommands(query: string, commands = COMMANDS): CommandMatch[] {
...
export function filterCommands(query: string, commands = COMMANDS): CommandDefinition[] {
  ...
  return commands
    ...
    .filter((entry): entry is { command: CommandDefinition; score: number } => entry.score !== null)
    .sort((left, right) => left.score - right.score || left.command.label.localeCompare(right.command.label, "zh-CN"))
    .map((entry) => entry.command);
}

export function exactCommandCandidates(input: string, commands = COMMANDS): CommandDefinition[] {
  const token = commandToken(input.trim().split(/\s+/, 1)[0] ?? "").toLowerCase();
  const exact = matchCommands(token, commands).filter((match) => match.matchedToken.toLowerCase() === token);
  return [...new Map(exact.map((match) => [match.canonicalId, match.command])).values()];
}

export function commandCompletion(input: string, commands = COMMANDS): CommandMatch | undefined {
