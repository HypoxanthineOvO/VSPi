...
export class OutputSpeedTracker {
  ...
  finish(outputTokens: number, durationMs: number): OutputSpeedSnapshot {
    if (Number.isFinite(durationMs) && durationMs > 0 && Number.isFinite(outputTokens) && outputTokens >= 0) {
  ...
  snapshot(): OutputSpeedSnapshot {
    ...
    const activeDurationMs = first ? Math.max(250, at - first.at) : 0;
