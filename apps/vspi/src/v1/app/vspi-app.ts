...
import {
	COMPACTION_PROFILES,
	type CompactOptions,
} from "../continuity/compaction-profiles.js";
...
import type { GoalLimits, StoredGoal } from "../goals/types.js";
import type {
	LocalPlanBackend,
	PlanBinding,
	PlanInput,
...
import { UserQuestionCancelledError } from "../questions/error.js";
import type {
	ExternalSessionSource,
	SessionLeaseOwner,
} from "../sessions/types.js";
import { normalizeSkillInstallSource } from "../skills/source.js";
...
import {
	renderActivityRail,
	renderQueuedMessage,
	type QueuedMessagePresentation,
} from "../ui/activity.js";
import { AgentsDock } from "../ui/agents-dock.js";
import { padLine, stripAnsi } from "../ui/ansi.js";
...
interface PromptProfileUi {
	...
	importFile(
		path: string,
		options: { scope: "global" | "project" | "session" },
	): Promise<unknown>;
...
export interface PlanTaskRouter {
	route(input: {
		...
		{ kind: "current-plan" } | { kind: "question"; questions: Question[] }
	>;
}
...
export type CompactionPresentationState =
	| {
...
export function reduceCompactionActivity(
	state: CompactionPresentationState | undefined,
	activity: CompactionActivity,
): CompactionPresentationState | undefined {
	...
	if (activity.type === "blocked") {
		if (state?.status !== "active" && state?.status !== "blocked") return state;
		return { ...state, status: "blocked", turnId: activity.turnId };
	}
	if (activity.type === "completed") {
		return { status: "completed", result: activity.result };
	}
	return { status: activity.type };
}
...
export class VspiApp implements Component, Focusable {
	...
	private readonly agentsDock = new AgentsDock();
	private taskSnapshot: TaskDashboardSnapshot = {
		agents: [],
		processes: [],
		questions: [],
	};
	...
	private queuedPresentations = new Map<
		string,
	...
	private agentDockTimer: NodeJS.Timeout | undefined;
	private agentConversationSubscription: BackendSubscription | undefined;
	private agentConversationLoadId = 0;
	private stableCommitTimer: NodeJS.Timeout | undefined;
	private waterfallSettling = false;
	...
	private fullscreenRenderRevision = 0;
	private fullscreenBodyCache:
		| { revision: number; width: number; lines: string[] }
		| undefined;
	private fullscreenDockCache:
		| { revision: number; width: number; dock: AppRenderDock }
		| undefined;
	private pendingFullscreenDock:
		| {
				width: number;
				dock: AppRenderDock;
	...
	set focused(value: boolean) {
		this._focused = value;
		this.composer.focused = value && this.panels.kind !== "question";
	}
	...
	constructor(
		...
		this.tui = tui;
		this.startupRuntimeDefaultsDiagnostic = options.startupRuntimeDiagnostic;
		const renderingTui = {
		...
		this.fullscreenDockSurface = this.createRenderSurface((width) =>
			this.renderFullscreenDock(width),
		);
		this.fullscreenScrollView = new ScrollView(
			this.fullscreenTranscriptSurface,
			{
				follow: "end",
				primary: true,
				overscroll: "chain",
		...
		this.fullscreenLayout = new VStack([
			{
				component: this.fullscreenScrollView,
				basis: 0,
				grow: 1,
				shrink: 1,
				minSize: 1,
			...
			{
				component: this.fullscreenDockSurface,
				basis: "auto",
				grow: 0,
				shrink: 1,
				minSize: 1,
		...
		this.bindTui(tui);
		this.thinkingTranslator =
			options.thinkingTranslator ?? new HttpThinkingTranslator();
		this.yoloAcknowledgementBroker =
			options.yoloAcknowledgementBroker ?? createYoloAcknowledgementBroker();
		...
		options.approvalBroker?.setHandler((request, signal) =>
		...
		this.composer.onAttachmentRemove = (attachment) =>
			void this.removeAttachment(attachment.id);
	}
	...
	async start(): Promise<void> {
		try {
			await this.backend.start({
				...
				onMessageUpdate: (id, patch) => {
					...
					if (index >= 0 && current) {
						...
						if (
							...
							this.agentsDock.settle(next.id);
						}
						if (
				...
				onCompactionActivity: (activity) => {
					if (this.sessionTransition) return;
					this.compaction = reduceCompactionActivity(this.compaction, activity);
					if (activity.type === "completed") {
						const result = activity.result;
						this.showNotice(
							`Context compacted ${String(result.tokensBefore)} → ${String(result.tokensAfter)} tokens · ${String(result.compactedCount)} messages`,
							"success",
						);
					}
					this.syncActivityPresentation();
				...
				onGoalChange: (goal) => {
					this.goalSnapshot = goal ? structuredClone(goal) : undefined;
					this.panels.setGoalSnapshot(
						this.goalSnapshot,
				...
				onAgentSnapshot: (snapshot) => {
					...
					this.agentsDock.setSnapshot(snapshot);
					this.syncAgentDockTimer();
					this.requestRender();
				},
				onTaskSnapshot: (snapshot) => {
					this.taskSnapshot = structuredClone(snapshot);
					this.panels.setTaskSnapshot(snapshot);
					this.requestRender();
				},
				onCronSnapshot: (tasks) => {
					this.scheduledCount = tasks.length;
				...
				onSessionReady: () => {
					...
					if (this.startupShellReady)
						void this.initializeRuntimeSurface().catch((error) =>
							this.handleRuntimeError(error),
						);
				},
				onRuntimeCatalogChanged: () => {
					void this.refreshRuntimeCatalog()
				...
				onHandoffInteraction: (interaction, signal) =>
					this.answerHandoffInteraction(interaction, signal),
				onHandoffProjection: (projection) =>
					this.applyHandoffProjection(projection),
				...
				onSessionInvalidating: () => {
					...
					this.cancelPendingApproval(
						"Approval cancelled because the session changed",
					);
				},
				onSessionReset: (session) => {
					...
					if (this.backend.getTaskSnapshot) {
						const snapshot = this.backend.getTaskSnapshot();
						this.taskSnapshot = structuredClone(snapshot);
						this.panels.setTaskSnapshot(snapshot);
					}
					const epoch = this.resetSessionState();
					const hydration = [this.refreshPlanSnapshot(epoch)];
					if (this.renderReady)
						hydration.push(this.switchAttachmentSession(session.id, epoch));
					if (hydrating) this.sessionHydrationTasks.push(...hydration);
					else for (const task of hydration) void task;
				},
			});
					...
					if (
						...
						(ownership.sessionId !== this.attachmentSessionId ||
							ownership.generation !==
								this.options.attachments.sessionGeneration)
					) {
						return;
					}
			...
			if (this.options.openOnStart === "sessions") {
				try {
					...
					this.showNotice(
						`会话读取失败：${error instanceof Error ? error.message : "未知错误"}`,
						"error",
					);
				}
	...
	private initializeRuntimeSurface(): Promise<void> {
		...
		this.runtimeSurfacePromise = (async () => {
			...
			const providers = runtimeProviders.map((provider) => {
				const source =
					catalog?.providers.find((item) => item.id === provider.id)?.source ??
					"builtin";
			...
			if (catalog && catalog.diagnostics.length > 0) {
				this.showNotice(
					catalog.diagnostics[0] ?? "Provider 配置诊断",
					"warning",
				);
			}
			...
			if (this.attachmentSessionId)
				await this.switchAttachmentSession(
	...
	async dispose(): Promise<void> {
		...
		if (this.agentDockTimer) clearInterval(this.agentDockTimer);
		this.disposeAgentConversation();
		if (this.stableCommitTimer) clearTimeout(this.stableCommitTimer);
		...
		try {
			...
			if (
				this.options.settings.summarizeSessionTitleOnExit &&
				this.backend.generateSessionTitle
			) {
				await waitForShutdownCancellation(
					this.backend.generateSessionTitle().catch(() => undefined),
				);
			}
	...
	private switchTuiMode(mode: TuiMode): boolean {
		...
		if (previous.mode === mode) {
			this.fullscreenScrollView.setScrollbar(
				this.options.settings.fullscreenScrollbar,
			);
			return true;
		}
		...
		if (previous instanceof ScrollbackTUI)
		...
		const next: TUI =
			mode === "fullscreen"
				? new VspiTuiAltScreen(terminal, showHardwareCursor, undefined, {
	...
	private async toggleTuiMode(): Promise<void> {
		const nextMode: TuiMode =
			this.options.settings.tuiMode === "fullscreen" ? "regular" : "fullscreen";
		try {
			...
			this.showNotice(
				nextMode === "fullscreen"
					? "已切换全屏模式（应用内滚动）"
					: "已切换常规模式（终端原生滚动，长历史更流畅）",
				"success",
			);
		} catch (error) {
			this.showNotice(
				error instanceof Error ? error.message : "切换 TUI 模式失败",
				"error",
			);
		}
	}
	...
	handleInput(data: string): void {
		...
		if (
			...
			(this.notice.tone === "warning" || this.notice.tone === "error")
		) {
		...
		const composerState = {
			...
			hasErrorDetails: this.messages.some(
		...
		if (selectedAttachment) {
			if (
				matchesInteraction("composer", "attachment", "renameAttachment", data)
			) {
			...
			if (
				matchesInteraction("composer", "attachment", "previewAttachment", data)
			) {
				void this.previewAttachment(
					selectedAttachment.id,
					selectedAttachment.alias,
				);
				return;
			}
			if (
				matchesInteraction("composer", "attachment", "saveAttachment", data)
			) {
		...
		if (
			...
			if (this.composer.getText().trim().length > 0)
				void this.interruptWithComposer();
			else void this.cancelGeneration();
			return;
		}
		if (
			matchesKey(data, Key.tab) &&
			composerState.composerEmpty &&
			!composerState.hasMessages
		) {
	...
	setStartupSurface(lines: readonly string[]): void {
		// Full-screen pickers own a fresh viewport. Keeping the splash in their
		// active frame makes the frame taller than the terminal and scrolls the
		// picker title above the visible area.
		this.startupSurface = this.panels.kind === "sessions" ? [] : [...lines];
		this.fullscreenRenderRevision += 1;
	}
	...
	private renderFullscreenBody(width: number): string[] {
		let pending = this.pendingFullscreenDock;
		if (!pending || pending.bodyRendered) {
			const terminalWidth = Number.isFinite(this.tui.terminal.columns)
				? this.tui.terminal.columns
				: width;
			pending = this.stageFullscreenDock(Math.max(1, terminalWidth));
		}
		pending.bodyRendered = true;
		let cached = this.fullscreenBodyCache;
		if (
			!cached ||
			cached.revision !== this.fullscreenRenderRevision ||
			cached.width !== width
		) {
			cached = {
				revision: this.fullscreenRenderRevision,
				width,
				lines: this.buildRenderBody(width, pending.dock),
			};
			this.fullscreenBodyCache = cached;
		}
		if (pending.dockRendered) this.pendingFullscreenDock = undefined;
		return cached.lines;
	}

	private renderFullscreenDock(width: number): string[] {
		let pending = this.pendingFullscreenDock;
		if (!pending || pending.width !== width || pending.dockRendered)
			pending = this.stageFullscreenDock(width);
		pending.dockRendered = true;
		const lines = pending.dock.lines;
		if (pending.bodyRendered) this.pendingFullscreenDock = undefined;
		return lines;
	}

	private stageFullscreenDock(
		width: number,
	): NonNullable<VspiApp["pendingFullscreenDock"]> {
		this.pendingFullscreenDock = undefined;
		let cached = this.fullscreenDockCache;
		if (
			!cached ||
			cached.revision !== this.fullscreenRenderRevision ||
			cached.width !== width
			...
			cached = {
				revision: this.fullscreenRenderRevision,
				width,
				dock: this.buildRenderDock(width),
			};
			this.fullscreenDockCache = cached;
		}
		const pending = {
			width,
			dock: cached.dock,
		...
		this.pendingFullscreenDock = pending;
		queueMicrotask(() => {
			if (this.pendingFullscreenDock === pending)
				this.pendingFullscreenDock = undefined;
		});
		return pending;
	}
	...
	private buildRenderSections(width: number): AppRenderSections {
		const dock = this.buildRenderDock(width);
		return { body: this.buildRenderBody(width, dock), dock: dock.lines };
	}

	private buildRenderDock(width: number): AppRenderDock {
		const questionActive = this.panels.kind === "question";
		const runtimeLine = this.renderRuntimeStatus(width);
		const runtime = [runtimeLine];
		this.composer.focused = this._focused && !questionActive;
		if (this.authDialog) {
			return {
				lines: appendRuntimeStatus([], runtimeLine),
		...
		if (this.panels.kind === "sessions") {
			...
			return {
				lines: appendRuntimeStatus(status, runtimeLine),
				activityActive: false,
				agents: [],
				composer: [],
				activity: [],
		...
		const composer = questionActive
			? []
			: this.composer.render(
					width,
					activityActive ? this.composerActivity() : undefined,
				);
		const activity =
			!questionActive &&
			activityActive &&
			this.options.settings.workingStyle === 1
				? [
						renderActivityRail(
							{
								...
								elapsedSeconds: this.activityElapsedSeconds(),
								...this.queueState,
							},
							width,
							this.theme,
		...
		const queuedMessages = questionActive
			...
			: [
					...
					return renderQueuedMessage(message, width, this.theme, {
						...
						frame: this.queuedAnimationTick,
		...
		const agents = questionActive
			? []
			: this.agentsDock.render(width, this.theme);
		return {
			lines: appendRuntimeStatus(
				[
					...activity,
					...queuedMessages,
					...agents,
	...
	private buildRenderBody(width: number, dock: AppRenderDock): string[] {
		if (this.authDialog)
			return this.withStartupSurface(this.authDialog.render(width, this.theme));
		if (this.panels.kind === "sessions") {
			const terminalRows = Number.isFinite(this.tui.terminal.rows)
				? this.tui.terminal.rows
				: 24;
			// Sessions owns the viewport but is vertically centered: content-sized surface with
			// remaining rows split above/below (at least 2 blank rows on top) so the panel title
			// never hugs row 0, which real terminals can hide behind their own chrome.
			const availableRows = Math.max(
				3,
				terminalRows - dock.status.length - dock.runtime.length,
			);
			const contentHeight = this.panels.sessionsContentHeight(
				width,
				this.theme,
			);
			const maxSurfaceRows = sessionsSurfaceRowLimit(availableRows);
			const surfaceRows = Math.min(contentHeight, maxSurfaceRows);
			const surface = this.panels.renderSessionsSurface(
				width,
				surfaceRows,
				this.theme,
			);
			const remaining = Math.max(0, availableRows - surface.length);
			...
			const blankBottom = Array.from({ length: bottomPad }, () =>
				padLine("", width),
			);
			return [...blank, ...surface, ...blankBottom];
		}
		const {
			activityActive,
			agents,
		...
		const interactiveSurfaceFocused =
			transcriptFocused || this.workspaceFocus === "plan";
		const planSurfaceVisible =
			!transcriptFocused &&
			(this.panels.kind !== "plan" ||
				this.panels.hasPlanContent() ||
				this.planPanelExplicit);
		const panelRows = this.panelRowBudget(
			composer.length,
			activity.length,
		...
		const panelHint =
			!previewLines && !transcriptFocused && planSurfaceVisible
				? this.renderPanelHint(width)
				: undefined;
		const auxiliaryRows =
			previewLines || transcriptFocused || panelHint !== undefined ? 1 : 0;
		const reserved = previewLines
			? previewLines.length + auxiliaryRows
			: (planSurfaceVisible ? panelRows : 0) + auxiliaryRows;
		const terminalRows = Number.isFinite(this.tui.terminal.rows)
			? this.tui.terminal.rows
			: 24;
		const questionGutterRows =
			!previewLines && this.panels.kind === "question" ? 1 : 0;
		...
		const output = renderTranscript(
			...
			this.theme,
			{
				...(transcriptFocused && this.inspectNodeId
					? { selectedNodeId: this.inspectNodeId }
					: {}),
				...(transcriptFocused &&
	...
	commitStableTranscript(): number {
		const commit = (
			this.tui as TUI & { commitStatic?: (lines: readonly string[]) => boolean }
		).commitStatic;
		if (!commit) return 0;
		let stableEnd = this.committedMessageCount;
		while (stableEnd < this.messages.length) {
			const message = this.messages[stableEnd];
			if (
				!message ||
				isQueuedTranscriptMessage(message) ||
				("streaming" in message && message.streaming === true)
			)
				break;
			stableEnd += 1;
		}
		if (stableEnd <= this.committedMessageCount) return 0;

		const width = Math.max(1, this.tui.terminal.columns);
		const stableMessages = this.messages.slice(
			this.committedMessageCount,
			stableEnd,
		);
		const visibleTail = selectTranscriptWindow(stableMessages, {
			width,
			maxRows: this.transcriptViewportRows(width),
			maxBlocks: 80,
		...
		const firstVisibleMessage =
			visibleTail.nodes[0]?.messageIndexes[0] ?? stableMessages.length;
		const commitEnd = this.committedMessageCount + firstVisibleMessage;
		const messages = this.messages.slice(this.committedMessageCount, commitEnd);
		const lines = renderTranscript(messages, width, this.theme, {
			thinkingDisplay: this.options.settings.thinkingDisplay,
			wrapCode: this.options.settings.wrapCode,
			collapseCompletedTools: this.options.settings.collapseTools,
			mermaidRendering: this.options.settings.mermaidRendering,
		...
		if (
			messages.length > 0 &&
			commitEnd < this.messages.length &&
			stripAnsi(lines.at(-1) ?? "") !== ""
		) {
			lines.push("");
		}
		const prefix = [...this.startupSurface, ...lines];
		if (prefix.length === 0 || commit.call(this.tui, prefix) !== true) return 0;
		const committed = commitEnd - this.committedMessageCount;
		this.committedMessageCount = commitEnd;
		this.startupSurface = [];
		return committed;
	}

	/** Start one append-only surface epoch after all restored Session state has hydrated. */
	private presentRestoredTranscript(
	...
	private beginTuiSurfaceEpoch(): void {
		const beginSurfaceEpoch = (
			this.tui as TUI & { beginSurfaceEpoch?: () => void }
		).beginSurfaceEpoch;
		if (beginSurfaceEpoch) beginSurfaceEpoch.call(this.tui);
	...
	private async finishSessionTransition(
		previousEpoch: number,
		mode: "append" | "replace" = "append",
	): Promise<void> {
		if (!this.sessionResetObserved && this.sessionEpoch === previousEpoch)
			this.resetSessionState();
		while (this.sessionHydrationTasks.length > 0) {
			const pending = this.sessionHydrationTasks.splice(0);
		...
		this.sessionResetObserved = false;
		this.clearNotice();
		this.presentRestoredTranscript(mode);
	}

	private abortSessionTransition(): void {
		this.sessionTransition = false;
		this.sessionResetObserved = false;
		this.sessionHydrationTasks = [];
		this.clearNotice();
	...
	private panelRowBudget(
		...
		return Math.min(
			this.panels.kind === "models"
				? 24
				: this.panels.kind === "question"
					? 22
					: 16,
			availableRows,
		);
	...
	private renderPanelHint(width: number): string | undefined {
		if (this.panels.kind === "plan" && !this.panelFocused) {
			return this.theme.muted(
				padLine(renderInteractionHint("panel", "plan", {}), width),
			);
		}
	...
	private renderInspectHint(): string {
		const hint = renderInteractionHint(
			"inspect",
			"transcript",
			this.inspectInteractionState(),
		);
		return this.panels.hasPlanContent()
			? hint
			: hint.replace("Shift+Tab 进入 Plan", "Shift+Tab 返回输入");
	}
	...
	private async submit(
		...
		if (text.startsWith("//")) {
			...
			if (command) {
				...
				if (
					queuedDuringWork &&
					(!action?.handler || !BUSY_SAFE_ACTIONS.has(action.handler))
				) {
					this.showNotice(
						"该命令需等待当前任务结束；Model、Effort、Policy 与界面设置可立即调整",
						"info",
					);
					return;
				}
		...
		const binding = queuedDuringWork
			? undefined
			: this.backend.getPlanBinding?.();
		if (
			binding &&
			this.options.planBackend &&
			this.planSnapshot?.id !== binding.planId
		) {
			await this.refreshPlanSnapshot(this.sessionEpoch);
			if (this.planSnapshot?.id !== binding.planId) {
				this.showNotice(
					"当前 Plan binding 尚未完成快照同步，请重试",
					"warning",
				);
				return;
			}
		...
		if (
			binding &&
			this.planSnapshot?.id === binding.planId &&
			this.options.planTaskRouter &&
			!options?.skipPlanRoute
		) {
		...
		if (pendingAttachments.length > 0 && !this.backend.supportsVision) {
			this.showNotice(
				`${this.modelLabel} 不支持图片输入，请先切换 vision 模型`,
				"error",
			);
			return;
		}
		...
		const delivery = queuedDuringWork
			? behavior === "followUp"
				? "followUp"
				: "steer"
			: undefined;
		...
		if (queuedDuringWork) {
			try {
				...
				this.composer.restoreDraft(
					[raw, currentDraft].filter(Boolean).join("\n\n"),
					[...attachments, ...this.composer.attachments],
				);
				this.showNotice(
					error instanceof Error ? error.message : "排队消息发送失败",
					"error",
				);
			}
			return;
		...
		try {
			...
			if (this.activeSubmission?.id === submission.id)
	...
	private renderStatus(width: number): string[] {
		let lines: string[];
		let noticeContext: string | undefined;
		if (this.renameAttachmentId) {
			lines = [
				this.theme.focus(
					padLine(
						`重命名  ${this.renameInput}${this.theme.inverse(" ")}`,
						width,
					),
				),
			];
			...
			if (selectedAttachment) {
				lines = [
					this.theme.blue(
						padLine(
							`〔${selectedAttachment.alias}〕  重命名 · 预览 · 移除 · 保存到项目`,
							width,
						),
					),
				];
	...
	private setNotice(text: string, tone: NoticeTone, progress: boolean): void {
		...
		if (this.noticeTimer) clearTimeout(this.noticeTimer);
		this.noticeTimer = undefined;
		if (!progress && (tone === "info" || tone === "success")) {
	...
	private compactionActive(): boolean {
		return (
			this.compaction?.status === "active" ||
			this.compaction?.status === "blocked"
		);
	}

	private workingActive(): boolean {
		return (
			this.runActive ||
			this.busy ||
			this.queueState.steering + this.queueState.followUp > 0 ||
			[...this.queuedPresentations.values()].some(
	...
	private activityLabel(): string {
		if (this.compaction?.status === "blocked")
			return "Compacting context · continuing when ready";
		return this.compactionActive() ? "Compacting context" : "Working";
	}

	private activityElapsedSeconds(): number {
		const compaction = this.compaction;
		const startedAt =
			compaction?.status === "active" || compaction?.status === "blocked"
				? compaction.startedAt
				: this.workingStartedAt;
		return startedAt === undefined ? 0 : (Date.now() - startedAt) / 1_000;
	}
	...
	private syncActivityPresentation(): void {
		...
		const timerInterval = reducedMotion ? 1_000 : 240;
		const workingActive = this.workingActive();
		if (workingActive && this.workingStartedAt === undefined)
			this.workingStartedAt = Date.now();
		else if (!workingActive) this.workingStartedAt = undefined;
		if (
			active &&
			timerInterval !== undefined &&
			(!this.workingTimer || this.workingTimerInterval !== timerInterval)
		) {
			if (this.workingTimer) clearInterval(this.workingTimer);
			this.workingTimerInterval = timerInterval;
			this.workingTimer = setInterval(() => {
				if (!this.activityReducedMotion()) {
					this.workingFrame = (this.workingFrame + 1) % 24;
					this.queuedAnimationTick += 1;
					this.advanceQueuedPresentations();
					this.syncActivityPresentation();
				}
				this.requestRender();
			}, timerInterval);
	...
	private syncAgentDockTimer(): void {
		if (this.agentsDock.hasActive()) {
			if (this.agentDockTimer) return;
			this.agentDockTimer = setInterval(() => this.requestRender(), 1_000);
			this.agentDockTimer.unref();
			return;
		}
		if (!this.agentDockTimer) return;
		clearInterval(this.agentDockTimer);
	...
	private async openAgentConversation(runId: string): Promise<void> {
		...
		if (!this.backend.getAgentConversation) {
			this.panels.setAgentConversationError(
				"当前后端不支持 Agent conversation",
			);
			return;
		}
		...
		try {
			...
			if (loadId !== this.agentConversationLoadId) return;
			this.panels.setAgentConversationError(
				error instanceof Error ? error.message : "Agent conversation 加载失败",
			);
		}
		this.requestRender();
	}
	...
	private async loadOlderAgentConversation(
		...
		try {
			...
			if (loadId !== this.agentConversationLoadId) return;
			this.panels.prependAgentConversation(page);
		} catch (error) {
			if (loadId !== this.agentConversationLoadId) return;
			this.showNotice(
				`较早对话加载失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		}
		this.requestRender();
	...
	private disposeAgentConversation(): void {
		this.agentConversationLoadId += 1;
		this.agentConversationSubscription?.dispose();
		this.agentConversationSubscription = undefined;
		this.panels.clearAgentConversation();
	}

	private scheduleStableTranscriptCommit(attempt = 0): void {
		if (this.stableCommitTimer) clearTimeout(this.stableCommitTimer);
		this.waterfallSettling = true;
		this.requestRender();
		this.stableCommitTimer = setTimeout(() => {
			this.stableCommitTimer = undefined;
			if (this.activityActive()) return;
			const boundedWindow = this.currentTranscriptWindow();
			const requiresRebase = boundedWindow.hiddenBlocks > 0;
			const committed = this.commitStableTranscript();
	...
	private async executeCommand(
		...
		if (action.availability === "disabled") {
			this.showNotice(
				action.disabledReason ?? `${action.label} 暂不可用`,
				"warning",
			);
			this.panels.close();
			return;
	...
	private async executeEnabledAction(
		...
		if (action.handler === "newSession") {
			try {
				...
				this.showNotice(
					`新建会话失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			return;
		...
		if (action.handler === "compact") {
			try {
				...
				if (compact === "list") {
					const defaultProfile = this.backend.getPlanBinding?.()
				...
				const started = await this.backend.compact(compact);
					if (!started)
						this.showNotice("Context compaction is already running", "warning");
				this.composer.setText("");
				this.panels.close();
			} catch (error) {
				this.composer.setText(raw);
				this.showNotice(
					`上下文压缩失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			return;
		...
		if (action.handler === "update") {
			...
			this.requestRender();
			try {
				const result = await (this.options.selfUpdate ?? updateVspi)(
					VSPI_VERSION,
				);
				this.showNotice(
		...
		if (action.handler === "reload") {
			if (this.runActive) {
				this.showNotice(
					"会话运行中，无法 /reload；请先等待完成或 ESC 中断",
					"warning",
				);
				return;
			}
			...
			try {
				const currentModel = this.currentModelIdentity
					? this.modelOptions.find(
							(model) =>
								model.provider === this.currentModelIdentity?.provider &&
								model.id === this.currentModelIdentity.id,
						)
					: undefined;
				const levels = (await this.backend.getEffortOptions?.()) ??
					currentModel?.efforts ?? ["off"];
				this.panels.openEffort(this.effort, levels);
			} catch (error) {
				this.showNotice(
					`Effort 读取失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			return;
		...
		if (action.handler === "models") this.panels.open("models");
			...
			try {
				const command = parseAgentsCommand(raw);
				if (command.kind === "model") {
					if (!this.backend.switchTeammateModel)
						throw new Error("当前后端不支持 Teammate 模型切换");
					await this.backend.switchTeammateModel(command.id, command.model);
					this.showNotice(
						`Teammate ${command.id} 已切换到 ${command.model}`,
						"success",
					);
				} else if (command.kind === "reset") {
					if (!this.backend.resetTeammateLane)
						throw new Error("当前后端不支持 Teammate lane 重置");
					await this.backend.resetTeammateLane(command.id, command.lane);
					this.showNotice(
						`Teammate ${command.id} · lane ${command.lane ?? "default"} 已重置`,
						"success",
					);
				} else if (command.kind === "pool") {
					if (!this.backend.setAgentPoolRole)
						throw new Error("当前后端不支持 Agent Pool 配置");
					await this.backend.setAgentPoolRole(
						command.provider,
						command.role,
						command.model,
					);
					this.showNotice(
						`${command.provider} · ${command.role} 已映射到 ${command.model}`,
						"success",
					);
				} else if (command.kind === "stop") {
					if (!this.backend.stopAgentTask)
						throw new Error("当前后端不支持停止 Agent task");
					...
					if (!this.backend.detachAgentTask)
						throw new Error("当前后端不支持 detached Agent task");
					...
					if (!this.backend.overrideRequiredTeammate)
						throw new Error("当前后端不支持 Teammate required override");
					await this.backend.overrideRequiredTeammate(
						command.id,
						command.scope,
					);
					this.showNotice(
						`Required routing override: ${command.id} · ${command.scope}`,
						"success",
					);
				}
			...
			try {
				...
				if (command.kind === "create") {
					if (!this.backend.createCronTask)
						throw new Error("当前后端不支持 Cron");
					const task = await this.backend.createCronTask({
						runAt: command.runAt,
						prompt: command.prompt,
					});
					this.showNotice(`Cron ${task.id} 已安排`, "success");
				} else if (command.kind === "cancel") {
					if (!this.backend.deleteCronTask)
						throw new Error("当前后端不支持 Cron");
					if (!(await this.backend.deleteCronTask(command.id)))
						throw new Error(`找不到 Cron ${command.id}`);
				...
				this.showNotice(
					`Cron 操作失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			this.focusPlan();
		} else if (action.handler === "goal") {
			await this.executeGoalCommand(raw);
			return;
			...
			try {
				...
				this.showNotice(
					`外部会话读取失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			try {
				...
				this.showProgress("正在读取 VSPi、Codex 与 Claude Code Skill…");
				...
				this.showNotice(
					`Skill 读取失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
		} else if (
			action.handler === "settings" ||
			action.handler === "thinkingSettings"
		) {
			try {
				...
				this.showNotice(
					`设置读取失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
	...
	private async applyPanelEvent(event: PanelEvent | undefined): Promise<void> {
		...
		if (event.type === "close") {
			...
			if (this.pendingQuestion)
				this.cancelPendingQuestion("Question cancelled by user", {
					userInitiated: true,
				});
		...
		} else if (event.type === "agentRefresh") {
			this.panels.setAgentSnapshot(
				this.backend.getAgentSnapshot?.() ?? {
					enabled: false,
					projectTrusted: false,
					...
					diagnostic: "当前后端不支持 Agent tasks",
				},
			);
			if (this.backend.getTaskSnapshot) {
				const snapshot = this.backend.getTaskSnapshot();
			...
			try {
				if (!this.backend.stopAgentTask)
					throw new Error("当前后端不支持停止 Agent task");
				await this.backend.stopAgentTask(event.taskId);
				const snapshot = this.backend.getAgentSnapshot?.();
				if (snapshot) this.panels.setAgentSnapshot(snapshot);
				this.showNotice(`Agent task ${event.taskId} 已停止`, "success");
			} catch (error) {
				this.showNotice(
					`Agent task 停止失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			try {
				if (
					!this.backend.selectModel ||
					!this.backend.setEffort ||
					!this.backend.getModelOptions
				) {
					throw new Error("当前后端不支持模型组切换");
				}
				const role =
					event.group.roles.find((candidate) => candidate.role === "默认") ??
					event.group.roles[0];
				if (!role) throw new Error(`模型组 ${event.group.label} 没有可用角色`);
				const matches = (await this.backend.getModelOptions()).filter(
					(model) => model.id === role.modelId,
				);
				if (matches.length !== 1 || !matches[0]?.provider) {
					throw new Error(
						`模型组 ${event.group.label} 的模型 ${role.modelId} 缺失或 Provider identity 不唯一`,
					);
				}
				...
				this.showNotice(
					`模型组切换失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
		} else if (event.type === "provider") {
			this.showNotice(
				`${event.provider.label} 请先从操作菜单选择离线检查、网络测试或编辑`,
				"info",
			);
			...
			if (event.action.startsWith("login:")) {
				...
				if (type === "oauth" || type === "api_key")
					await this.startProviderLogin(event.provider, type);
				...
				await this.runProviderProbe(
					event.provider.id,
					"minimal-generation",
					async () => event.costConfirmed === true,
				);
			}
		} else if (event.type === "providerSave") {
			try {
				if (!this.providerConfig || !this.providerCatalogHash)
					throw new Error("Provider overlay service 未配置");
				const saved = await this.providerConfig.saveProjectProvider(
					event.provider.id,
					event.value,
					{
				...
				this.showNotice(
					`Provider 配置未保存：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			if (event.session.owner) {
				try {
					...
					if (action === "fork") {
						if (!this.backend.forkSession)
							throw new Error("当前后端不支持会话分支");
						const epoch = this.beginSessionTransition();
						this.showProgress("正在创建会话分支…");
					...
					this.abortSessionTransition();
					this.showNotice(
						`Session 操作失败：${error instanceof Error ? error.message : "未知错误"}`,
						"error",
					);
					return;
				}
			...
			try {
				...
				this.abortSessionTransition();
				this.showNotice(
					`会话切换失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			try {
				...
				this.abortSessionTransition();
				this.showNotice(
					`会话分支失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			try {
				if (
					!this.backend.previewExternalSession ||
					!this.backend.importExternalSession
				) {
					throw new Error("当前后端不支持外部会话导入");
				}
				this.showProgress("正在读取外部会话的完整可见记录…");
				const preview = await this.backend.previewExternalSession(
					event.session.id,
				);
				const contextWarning =
					this.usage.contextWindow > 0 &&
					preview.estimatedTokens > this.usage.contextWindow * 0.8
						? `导入内容超过当前 ${formatTokenEstimate(this.usage.contextWindow)} 上下文的 80%，首次继续时可能触发压缩。`
						: undefined;
				const epoch = this.sessionEpoch;
				this.beginSessionTransition();
				await this.backend.importExternalSession(
					event.session.id,
					preview.fingerprint,
				);
				await this.finishSessionTransition(epoch);
				...
				this.showNotice(
					`会话导入失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
		} else if (event.type === "skillInstall") {
			try {
				if (!this.backend.installSkill || !this.backend.listSkills)
					throw new Error("当前后端不支持 Skill 安装");
						...
						options: [
							{
								id: "install-enable",
								label: "安装并启用",
								description: "保存 Skill 包记录并加载发现的 Skill",
							},
							{
								id: "install-only",
								label: "仅安装",
								description: "保存包记录，但暂不加载任何 Skill",
							},
							{
								id: "cancel",
								label: "取消",
								description: "不安装、不修改设置",
							},
						],
					},
				]);
				if (
					answered?.answer !== "install-enable" &&
					answered?.answer !== "install-only"
				) {
					this.panels.open("skills");
					return;
				}
				this.showProgress("正在安装 Skill 包…");
				const result = await this.backend.installSkill(
					source,
					event.scope,
					answered.answer === "install-enable",
				);
				...
				this.panels.open("skills");
				this.showNotice(
					`${result.enabled ? "已安装并启用" : "已安装"} ${result.skills.length} 个 Skill`,
					"success",
				);
			} catch (error) {
				if (error instanceof Error && error.name === "AbortError") {
				...
				this.panels.open("skills");
				this.showNotice(
					`Skill 安装失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			try {
				const settings = {
					...
					thinkingTranslationEndpoint: normalizeTranslationEndpoint(
						event.settings.thinkingTranslationEndpoint,
					),
				};
				...
				this.showNotice(
					`${settings.scope === "global" ? "全局" : "项目"}设置已保存到 ${path}`,
					"success",
				);
			} catch (error) {
				this.showNotice(
					`设置保存失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			try {
				...
				this.showNotice(
					`Effort 切换失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			try {
				if (!this.options.promptProfiles)
					throw new Error("Prompt Profile service 未配置");
				...
				this.showNotice(
					`Prompt Profile Fork 失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
			...
			try {
				if (!this.options.promptProfiles)
					throw new Error("Prompt Profile service 未配置");
				await this.options.promptProfiles.importFile(event.path, {
					scope: event.scope,
				});
				await this.refreshPromptPanel();
				this.showNotice(
					`Prompt Profile 已从 ${event.path} 导入 Session`,
					"success",
				);
			} catch (error) {
				this.showNotice(
					`Prompt Profile 导入失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
		} else if (event.type === "promptExport") {
			try {
				const path = await this.options.promptProfiles?.writeExport(
					event.profileId,
				);
				...
				this.showNotice(
					`Prompt Profile 导出失败：${error instanceof Error ? error.message : "未知错误"}`,
					"error",
				);
			}
		} else {
	...
	private async openLogin(raw: string): Promise<void> {
		...
		const method =
			provider.authMethods?.find((candidate) => candidate.type === "oauth") ??
	...
	private async startProviderLogin(
		...
		try {
			...
			this.showNotice(
				type === "oauth"
					? `${provider.label} 账号已连接`
					: `${provider.label} API Key 已保存`,
				"success",
			);
		} catch (error) {
			if (cancelled || dialog.signal.aborted) return;
	...
	private async removeProviderCredential(
		provider: ProviderOption,
	): Promise<void> {
		if (!provider.storedCredential) {
			this.showNotice(
				`${provider.label} 没有由 VSPi 保存的凭据；环境变量不会被移除`,
				"warning",
			);
			return;
		}
		try {
			if (!this.backend.logoutProvider)
				throw new Error("当前后端不支持移除 Provider 凭据");
			await this.backend.logoutProvider(provider.id);
			await this.refreshRuntimeCatalog();
			this.panels.open("providers");
			this.panels.selectProvider(provider.id);
			this.showNotice(
				`${provider.label} 的已保存凭据已移除；环境变量和 models.json 未改变`,
				"success",
			);
		} catch (error) {
			this.showNotice(
				`移除凭据失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		}
	}
	...
	private async refreshPlanSnapshot(epoch: number): Promise<void> {
		...
		if (!binding || !this.options.planBackend) {
			if (
				...
				this.planSnapshot = undefined;
				if (!this.options.workflowAdapter)
					this.panels.setPlanSnapshot(undefined);
				this.requestRender();
			}
			return;
		}
		try {
			...
			if (!this.options.workflowAdapter)
				this.panels.setPlanSnapshot(this.planSnapshot);
			if (!plan)
				this.showNotice(
					`绑定的 Local Plan ${binding.planId} 不存在`,
					"warning",
				);
			this.requestRender();
		} catch (error) {
			...
			this.showNotice(
				`Local Plan 读取失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
			this.requestRender();
		}
	...
	private async refreshWorkflowSnapshot(): Promise<
	...
	private async refreshPromptPanel(): Promise<void> {
		...
		const runtimeSegments =
			this.backend.getEffectivePromptSegments?.() ??
		...
		this.panels.setPromptSnapshot({
			...
			resolved: {
				...
				pinned: Boolean(
					snapshot.session?.pin ?? snapshot.project?.pin ?? snapshot.global.pin,
				),
	...
	private async executeGoalCommand(raw: string): Promise<void> {
		try {
			const command = parseGoalCommand(raw);
			if (command.kind === "start") {
				if (this.activityActive())
					throw new Error("当前任务仍在运行；请先完成或取消后再创建 Goal");
				if (!this.backend.createGoal) throw new Error("当前后端未启用 Goal");
				const goal = await this.backend.createGoal(
					command.request,
					command.limits,
				);
			...
			if (command.kind === "status") {
				this.goalSnapshot = (await this.backend.getGoal?.()) ?? undefined;
				this.panels.setGoalSnapshot(this.goalSnapshot, this.backend.modelLabel);
				this.panels.open("goal");
				this.focusComposer();
				this.requestRender();
				return;
			}
			if (command.kind === "resume" && this.activityActive()) {
				throw new Error(
	...
	private async executePromptCommand(raw: string): Promise<void> {
		...
		try {
			...
			if (operation === "import") {
				...
				const profileId =
					rest[0] ?? service.resolve(this.currentPromptIdentity()).profileId;
				if (!profileId) throw new Error("用法：/prompt export <profile-id>");
				const path = await service.writeExport(profileId);
				this.showNotice(`Prompt Profile 已导出到 ${path}`, "success");
			} else if (operation) {
				throw new Error(
					"用法：/prompt [import <schema-file> | export <profile-id>]",
				);
			}
			await this.refreshPromptPanel();
			this.panels.open("prompt");
			...
			this.showNotice(
				`Prompt Profile 操作失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		}
	}
	...
	private currentPromptIdentity(): ModelIdentity {
		return this.currentModelIdentity
			? {
					provider: this.currentModelIdentity.provider,
					model: this.currentModelIdentity.id,
				}
			: {
					provider: this.backend.modelProvider ?? "unknown",
					model: this.backend.modelId,
				};
	}
	...
	private async updatePromptSession(input: {
		...
		try {
			...
			if (input.toggleRule) {
				const scope =
					input.toggleRule.ownerScope ??
					findPromptRuleScope(snapshot, input.toggleRule.ruleId);
				if (!scope)
					throw new Error(`Prompt rule ${input.toggleRule.ruleId} 不存在`);
				const layer =
					scope === "session"
						? snapshot.session
						: scope === "project"
							...
							: snapshot.global;
				if (
					!layer?.rules.some((rule) => rule.id === input.toggleRule?.ruleId)
				) {
					throw new Error(
						`Prompt rule ${scope}/${input.toggleRule.ruleId} 不存在`,
					);
				}
				await service.save(
					scope,
					{
						...
						rules: layer.rules.map((rule) =>
							rule.id === input.toggleRule?.ruleId
								? { ...rule, enabled: input.toggleRule.enabled }
								: rule,
						),
					},
			...
			if (
				...
				/expectedHash|conflict|stale/i.test(error.message)
			) {
				await this.refreshPromptPanel().catch(() => undefined);
			}
			this.showNotice(
				`Prompt Profile 更新失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		}
	}
	...
	private async applyPlanEdit(
		event: Extract<PanelEvent, { type: "planEdit" }>,
	): Promise<void> {
		if (
			!this.options.planBackend ||
			!this.planSnapshot ||
			this.planSnapshot.id !== event.planId
		) {
		...
		try {
			...
			this.showNotice(
				`Local Plan 更新失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
			await this.refreshPlanSnapshot(this.sessionEpoch);
		}
	...
	private async applySkillToggle(
		...
		try {
			...
			if (
				enabled &&
				(skill.source === "codex" || skill.source === "claude") &&
				!skill.installed
			) {
				const options = [
					{
						id: "user",
						label: "Global 启用",
						description: "登记原路径到 VSPi 全局 Skill 设置",
					},
			...
			this.panels.open("skills");
			this.showNotice(
				`Skill 操作失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		}
	}

	private async applySkillMutation(
		action: "update" | "remove",
		skill: SkillCatalogItem,
	): Promise<void> {
		try {
			if (
				!this.backend.listSkills ||
				(action === "update"
					? !this.backend.updateSkill
					: !this.backend.removeSkill)
			) {
				throw new Error("当前后端不支持 Skill 管理");
			}
			const [answered] = await this.requestQuestions([
			...
			this.panels.open("skills");
			this.showNotice(
				`${skill.name} 已${action === "update" ? "更新" : "移除"}`,
				"success",
			);
		} catch (error) {
			if (error instanceof Error && error.name === "AbortError") return;
	...
	private requestQuestions(
		...
		return new Promise<Question[]>((resolve, reject) => {
			...
			if (this.sessionHandoffRelay)
				void this.relayPendingQuestion(this.pendingQuestion);
			...
			const [answer] = await this.requestQuestions([
				{
					...
					options: [
						{
							id: isKill ? "kill" : "terminate",
							label: isKill ? "发送 SIGKILL 并接管" : "终止进程并接管",
						},
						{
							id: "cancel",
							label: "取消",
							description: "保留原进程，不接管 Session",
						},
					],
				},
			]);
			return answer?.answer === (isKill ? "kill" : "terminate")
				? isKill
					? "kill"
					: "terminate"
				: "cancel";
				...
				options: [
					{
						id: "takeover",
						label: "接管此会话",
						description: "默认；等待安全点后继续同一线程",
					},
					{
						id: "fork",
						label: "创建分支",
						description: "保留当前 owner，创建独立 Session",
					},
					{
						id: "cancel",
						label: "取消",
						description: "返回 Sessions，不改变任何进程",
					},
				],
			},
		]);
		return answered?.answer === "fork"
			? "fork"
			: answered?.answer === "cancel"
				? "cancel"
				: "takeover";
	}
	...
	private cancelPendingQuestion(
		message: string,
		options: { userInitiated?: boolean; abort?: boolean } = {},
	): void {
		const pending = this.pendingQuestion;
		if (!pending) return;
		this.pendingQuestion = undefined;
		...
		const error = options.userInitiated
			? new UserQuestionCancelledError(message)
			: new Error(message);
	...
	private requestApproval(
		...
		if (this.pendingApproval)
			return Promise.resolve({
				type: "deny",
				reason: "Another approval is already active",
			});
		return new Promise<ApprovalResponse>((resolve, reject) => {
			...
			if (this.sessionHandoffRelay)
				void this.relayPendingApproval(this.pendingApproval);
	...
	private beginSessionHandoff(relay: SessionHandoffRelay): void {
		...
		relay.project({
			...
			...(this.backend.modelProvider
				? { modelProvider: this.backend.modelProvider }
				: {}),
		...
		if (this.panels.kind === "question" || this.panels.kind === "approval")
			this.panels.close();
		if (this.pendingQuestion)
			void this.relayPendingQuestion(this.pendingQuestion);
		if (this.pendingApproval)
	...
	private cancelSessionHandoff(): void {
		...
		if (this.foregroundRelinquished) {
			this.messages = this.messages.filter(
				(message) => message.id !== "session-handoff-foreground",
			);
	...
	private applyHandoffProjection(projection: SessionHandoffProjection): void {
		if (projection.kind === "snapshot-start") {
			...
			this.messages.push(
				this.withThinkingDisplayDefault(structuredClone(projection.message)),
			);
		} else if (projection.kind === "message-update") {
			const index = this.messages.findIndex(
				(message) => message.id === projection.id,
			);
			const current = this.messages[index];
			if (index >= 0 && current) {
				this.messages[index] = {
					...current,
					...structuredClone(projection.patch),
	...
	private async relayPendingQuestion(pending: PendingQuestion): Promise<void> {
		...
		try {
			...
			if (response.kind !== "question")
				throw new Error("Session handoff returned the wrong interaction type");
			...
			pending.reject(
				error instanceof Error
					? error
					: new Error("Session handoff Question failed"),
			);
		}
	}
	...
	private async relayPendingApproval(pending: PendingApproval): Promise<void> {
		...
		try {
			...
			pending.reject(
				error instanceof Error
					? error
					: new Error("Session handoff Approval failed"),
			);
		}
	}
	...
	private async answerHandoffInteraction(
		...
		if (interaction.kind === "question") {
			const questions = await this.requestQuestions(
	...
	private async runProviderProbe(
		...
		try {
			if (!this.backend.runProviderProbe)
				throw new Error("当前后端不支持 Provider probe");
			const result = await this.backend.runProviderProbe(
				providerId,
				mode,
				confirmCost,
			);
			...
			this.showNotice(
				`Provider probe 失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		}
	}
	...
	private async persistRuntimeDefaults(): Promise<boolean> {
		...
		try {
			await this.runtimeDefaults.save(this.options.settings.scope, {
				...(this.currentModelIdentity
					? { model: this.currentModelIdentity }
					: {}),
				effort: this.effort,
			});
			return true;
	...
	private async applyRuntimeDefaults(
		...
		if (this.backend.setEffort) {
			try {
				...
				diagnostics.push(
					`默认 Effort 未应用：${error instanceof Error ? error.message : "未知错误"}`,
				);
			}
		}
	...
	private currentTranscriptWindow(
		width?: number,
		maxRows?: number,
		unboundedActiveTail = false,
	): TranscriptWindow {
		const terminalWidth = width ?? this.tui.terminal.columns;
		const safeWidth = Number.isFinite(terminalWidth)
			? Math.max(1, terminalWidth)
			: 80;
		const source =
			this.workspaceFocus === "transcript"
				? this.messages
				: this.messages.slice(this.committedMessageCount);
		return selectTranscriptWindow(source, {
			width: safeWidth,
			maxRows: unboundedActiveTail
				? Number.MAX_SAFE_INTEGER
				: Math.max(1, maxRows ?? this.transcriptViewportRows(safeWidth)),
			maxBlocks: unboundedActiveTail ? Number.MAX_SAFE_INTEGER : 80,
			...
			...(this.workspaceFocus === "transcript" && this.inspectNodeId
				? { pinnedNodeId: this.inspectNodeId }
				: {}),
	...
	private reconcileTranscriptStart(): void {
		const all = buildTranscriptNodes(this.messages);
		const selectedGlobal = all.findIndex(
			(node) => node.id === this.inspectNodeId,
		);
		...
		if (window.nodes.some((node) => node.id === this.inspectNodeId)) {
			if (window.truncatedTailBlocks === 0)
				this.transcriptStartNodeId = undefined;
			return;
		}
	...
	private transcriptViewportRows(width: number): number {
		const terminalRows = Number.isFinite(this.tui.terminal.rows)
			? this.tui.terminal.rows
			: 24;
		if (this.workspaceFocus !== "transcript" && this.workspaceFocus !== "plan")
			return terminalRows * 3;
		const activityActive = this.activityActive();
		const composerRows = this.composer.render(
			width,
			activityActive ? this.composerActivity() : undefined,
		).length;
		const activityRows =
			activityActive && this.options.settings.workingStyle === 1 ? 1 : 0;
		const queuedRows = this.messages.filter(isQueuedTranscriptMessage).length;
		const agentRows = this.agentsDock.render(width, this.theme).length;
		...
		const planSurfaceVisible =
			...
			(this.panels.kind !== "plan" ||
				this.panels.hasPlanContent() ||
				this.planPanelExplicit);
		const previewRows = this.preview
			? this.preview.render(width).length + 1
			: 0;
		const panelRows =
			previewRows === 0 && planSurfaceVisible
				...
				: 0;
		const questionGutterRows =
			previewRows === 0 && this.panels.kind === "question" ? 1 : 0;
		const chrome =
			this.startupSurface.length +
			composerRows +
			activityRows +
	...
	private withStartupSurface(lines: string[]): string[] {
		return this.startupSurface.length > 0
			? [...this.startupSurface, ...lines]
			: lines;
	}
	...
	private focusTranscript(): boolean {
		...
		const selected =
			...
			nodes.find(
				(node) =>
					this.inspectIndex !== undefined &&
					node.messageIndexes.includes(this.inspectIndex),
			) ??
			nodes.at(-1);
		...
		const selectedToolIndex = selected.messageIndexes.find(
			(index) =>
				this.messages[index]?.kind === "tool" &&
				this.messages[index]?.id === this.inspectToolId,
		);
		if (
			selected.kind !== "toolGroup" ||
			this.inspectDepth !== "tool" ||
	...
	private cycleWorkspaceFocus(): void {
		...
		if (this.workspaceFocus === "composer") {
			if (buildTranscriptNodes(this.messages).length > 0)
				this.focusTranscript();
	...
	private handleInspectInput(data: string): void {
		...
		if (
			interaction.handler === "moveInspect" &&
		...
		if (interaction.handler === "moveInspect") {
			...
			if (offset < 0 && nodeIndex === 0) {
				...
				if (!this.extendTranscriptHistory())
					this.showNotice("已到最早内容", "info");
				this.requestRender();
				return;
			}
			if (offset > 0 && nodeIndex === nodes.length - 1) {
				...
				const nextGlobal =
					selectedGlobal >= 0 ? all[selectedGlobal + 1] : undefined;
				if (nextGlobal) {
			...
			const next =
				nodes[Math.max(0, Math.min(nodes.length - 1, nodeIndex + offset))];
			if (next) {
			...
			if (
				...
				(matchesKey(data, Key.right) || matchesKey(data, Key.enter))
			) {
				...
				if (firstTool?.kind === "tool") {
					...
					this.inspectIndex = selectedNode.messageIndexes.find(
						(index) => this.messages[index]?.id === firstTool.id,
					);
				}
	...
	private pageTranscript(direction: -1 | 1): void {
		...
		const windowNodes = this.currentTranscriptWindow().nodes;
		if (windowNodes.length === 0) return;
		const firstGlobal = all.findIndex((node) => node.id === windowNodes[0]?.id);
		const lastGlobal = all.findIndex(
			(node) => node.id === windowNodes.at(-1)?.id,
		);
		const pageSize = Math.max(1, windowNodes.length);
		let targetGlobal: number;
		if (direction < 0) {
			// 上翻：选中在窗口内时从窗口顶部出发，翻到顶部前一个节点；否则按窗口大小继续上翻。
			const base =
				firstGlobal >= 0 && selectedGlobal >= firstGlobal
					? firstGlobal
					: selectedGlobal;
			targetGlobal = Math.max(0, base - pageSize);
		} else {
			// 下翻：选中在窗口内时从窗口底部出发，翻到底部后一个节点。
			const base =
				lastGlobal >= 0 && selectedGlobal <= lastGlobal
					? lastGlobal
					: selectedGlobal;
			targetGlobal = Math.min(all.length - 1, base + pageSize);
		}
		...
		this.inspectDepth = "node";
		this.inspectIndex = target.messageIndexes[0];
		this.transcriptStartNodeId =
			targetGlobal >= all.length - 1 ? undefined : target.id;
	}

	private handleToolInspectInput(data: string, node: TranscriptNode): void {
		const toolIndexes = node.messageIndexes.filter(
			(index) => this.messages[index]?.kind === "tool",
		);
		...
		const currentIndex = Math.max(
			0,
			toolIndexes.findIndex(
				(index) => this.messages[index]?.id === this.inspectToolId,
			),
		);
		if (matchesKey(data, Key.up) || matchesKey(data, Key.down)) {
			...
			const nextIndex =
				toolIndexes[
					Math.max(0, Math.min(toolIndexes.length - 1, currentIndex + offset))
				];
	...
	private withThinkingDisplayDefault(
		message: TranscriptMessage,
	): TranscriptMessage {
		if (message.kind !== "thinking") return message;
		return {
			...message,
			collapsed: this.options.settings.thinkingDisplay !== "expanded",
		};
	}
	...
	private queueVisibleThinkingTranslations(): void {
		...
		for (const message of visible.slice(-20))
			this.queueThinkingTranslation(message.id);
	}
		...
		this.thinkingTranslationQueue = this.thinkingTranslationQueue
			...
			.then(async () => {
				if (
					...
					revision !== this.thinkingTranslationRevision
				)
					return;
				const controller = new AbortController();
				this.thinkingTranslationAbort = controller;
				try {
					const translatedText = await this.thinkingTranslator.translate(
						sourceText,
						endpoint,
						controller.signal,
					);
					...
					if (this.thinkingTranslationAbort === controller)
						this.thinkingTranslationAbort = undefined;
				}
			});
		this.requestRender();
	...
	private updateThinkingTranslation(
		...
		patch: Pick<
			...
			"translatedText" | "translationStatus"
		>,
	): void {
		const index = this.messages.findIndex((message) => message.id === id);
		const current = this.messages[index];
	...
	private async pasteAttachment(): Promise<void> {
		try {
			...
			this.showNotice(
				`图片粘贴失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		}
	}
	...
	private async interruptWithComposer(): Promise<void> {
		...
		this.composer.setText("");
		this.requestRender();
		await this.cancelGeneration({ text, attachments });
	}

	private async cancelGeneration(interrupted?: {
		text: string;
		attachments: Attachment[];
	}): Promise<void> {
		...
		try {
			...
			cancelled = true;
			this.showNotice(
				queuedMessages.length > 0 || interrupted !== undefined
					? `已中断当前运行，正在提交 ${queuedMessages.length + (interrupted === undefined ? 0 : 1)} 条输入`
			...
			if (interrupted !== undefined) {
				const currentDraft = this.composer.getText().trim();
				this.composer.restoreDraft(
					[interrupted.text, currentDraft].filter(Boolean).join("\n\n"),
					[...interrupted.attachments, ...this.composer.attachments],
				);
			}
			this.showNotice(
				`取消生成失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		} finally {
		...
		if (cancelled && (queuedMessages.length > 0 || interrupted !== undefined))
			this.startInterruptedQueue(queuedMessages, interrupted);
	}
	...
	private async switchAttachmentSession(
		sessionId: string,
		epoch: number,
	): Promise<void> {
		const switchSession = (
			this.options.attachments as Partial<
				Pick<AttachmentService, "switchSession">
			>
		).switchSession;
		if (!switchSession) return;
		try {
			await switchSession.call(this.options.attachments, sessionId);
			if (this.sessionEpoch !== epoch || this.attachmentSessionId !== sessionId)
				return;
			this.composer.restoreAttachments(this.options.attachments.store.list());
		} catch {
			if (this.sessionEpoch === epoch)
				this.showNotice("附件会话恢复失败", "error");
		}
	}
	...
	private startInterruptedQueue(
		queuedMessages: string[],
		interrupted?: { text: string; attachments: Attachment[] },
	): void {
		const restoredAttachments: Attachment[] = [
			...(interrupted?.attachments ?? []),
		];
		...
		const text = [
			...queuedMessages,
			...(interrupted === undefined ? [] : [interrupted.text]),
		].join("\n\n");
		void this.sendInterruptedQueue(text, attachments);
	}

	private async sendInterruptedQueue(
		text: string,
		attachments: Attachment[],
	): Promise<void> {
		const transcriptLength = this.messages.length;
		...
		try {
			...
			const restoredAttachments = [
				...
				(attachment, index, all) =>
					all.findIndex((candidate) => candidate.id === attachment.id) ===
					index,
			);
			this.composer.restoreDraft(
				[text, currentDraft].filter(Boolean).join("\n\n"),
				restoredAttachments,
			);
			this.showNotice(
				error instanceof Error ? error.message : "排队消息重新发送失败",
				"error",
			);
		} finally {
			if (this.activeSubmission?.id === submission.id)
	...
	private async removeAttachment(id: string): Promise<void> {
		try {
			...
			this.showNotice(
				`附件移除失败：${error instanceof Error ? error.message : "未知错误"}`,
				"error",
			);
		}
	}
	...
	private handleRenameInput(data: string): void {
		...
		if (
			interaction.handler === "deleteRenameCharacter" ||
			interaction.handler === "editRename"
		) {
			if (this.renameField.getValue() !== this.renameInput)
				this.renameField.setValue(this.renameInput);
			this.renameField.handleInput(data);
			if (Array.from(this.renameField.getValue()).length > 200) {
				this.renameField.setValue(
					Array.from(this.renameField.getValue()).slice(0, 200).join(""),
				);
			}
	...
	private async commitAttachmentRename(id: string): Promise<void> {
		try {
			const attachment = await this.options.attachments.rename(
				id,
				this.renameInput,
			);
	...
	private async saveAttachment(id: string): Promise<void> {
		try {
			const path = await this.options.attachments.saveToProject(
				id,
				this.options.cwd,
			);
	...
	private requestRender(force = false): void {
		...
		if (
			...
			(!this.sessionTransition ||
				...
				this.panels.kind === "sessions")
		)
			this.tui.requestRender(force);
	}

	private refreshModelPresentation(): void {
		if (this.startupSurface.length > 0) {
			const width = Math.max(4, this.tui.terminal.columns ?? 80);
			this.startupSurface = renderSplash(
				width,
				this.theme,
				1,
				this.startupStatus(),
		...
		if (this.goalSnapshot)
			this.panels.setGoalSnapshot(this.goalSnapshot, this.modelLabel);
	}
}
...
export function parseAgentsCommand(
	...
	if (parts[1] === "model" || parts[1] === "reset" || parts[1] === "override") {
		throw new Error(
			"Teammate 暂不可用（后续版本回归）；/tasks pool 仍可配置 Agent Pool",
		);
	}
	if (parts[1] === "stop" && parts.length === 3)
		return { kind: "stop", taskId: parts[2] as string };
	if (parts[1] === "detach" && parts.length === 3)
		return { kind: "detach", taskId: parts[2] as string };
	if (
		parts[1] === "pool" &&
		parts.length === 5 &&
		...
		return {
			kind: "pool",
			provider: parts[2] as string,
			role: parts[3] as AgentRole,
			model: parts[4] as string,
		};
	}
	throw new Error(
		"用法：/tasks、/tasks stop|detach <task-id>、/tasks pool <provider> <orchestrator|researcher|analyst|worker> <provider/model>",
	);
}
...
export function parseCronCommand(raw: string, now = Date.now()): CronCommand {
	...
	if (operation === "wake") {
		if (!first || rest.length > 0)
			throw new Error("用法：/cron wake <duration>，例如 /cron wake 2h");
		return {
			kind: "create",
			runAt: now + parseCronDuration(first),
			prompt: CRON_WAKE_PROMPT,
		};
	}
	if (operation === "in") {
		const prompt = rest.join(" ").trim();
		if (!first || !prompt)
			throw new Error("用法：/cron in <duration> <prompt>");
		return { kind: "create", runAt: now + parseCronDuration(first), prompt };
	}
	if (operation === "at") {
		const prompt = rest.join(" ").trim();
		if (!first || !prompt)
			throw new Error("用法：/cron at <ISO date-time> <prompt>");
...
type GoalCommand =
	| { kind: "status" | "pause" | "resume" | "cancel" | "accept" }
	| { kind: "start"; request: string; limits: Partial<GoalLimits> };

function parseGoalCommand(raw: string): GoalCommand {
	let rest = raw
		.trim()
		.replace(/^\/goal(?:\s+|$)/, "")
		.trim();
	...
	if (
		...
		return { kind: rest };
	const limits: Partial<GoalLimits> = {};
	const options: Array<[string, keyof GoalLimits, number, number]> = [
		["rounds", "maxAutoRounds", 1, 1_000],
		["no-progress", "maxNoProgressRounds", 1, 100],
		["tokens", "maxTokens", 1_000, 100_000_000],
	];
	for (;;) {
		let matched = false;
		for (const [name, key, minimum, maximum] of options) {
	...
	if (!rest) {
		throw new Error(
			"用法：/goal [--rounds N] [--no-progress N] [--tokens N] <request>",
		);
	}
	if (rest.startsWith("--"))
		throw new Error(`未知 Goal 参数：${rest.split(/\s+/, 1)[0]}`);
	return { kind: "start", request: rest, limits };
}
...
function parseCompactOptions(raw: string): CompactOptions | "list" | undefined {
	...
	if (profile === "native" || profile === "pi-native")
		return { profile: "pi-native" };
	if (profile === "continuity" || profile === "execution-continuity")
		return { profile: "execution-continuity" };
	if (profile === "research" || profile === "research-decisions")
		return { profile: "research-decisions" };
	if (profile === "custom") {
		const customInstructions = rest.join(" ").trim();
		if (!customInstructions)
			throw new Error("用法：/compact custom <instructions>");
		return { profile: "custom", customInstructions };
	}
	throw new Error(
		"用法：/compact [--list | native | continuity | research | custom <instructions>]",
	);
}
...
function publicPlanInput(plan: StoredPlan): PlanInput & { archived: boolean } {
	return {
		...
		...(plan.focusItemId === undefined
			? {}
			: { focusItemId: plan.focusItemId }),
...
function updateItemStatus(
	...
	for (const item of items) {
		...
		if (item.children && updateItemStatus(item.children, itemId, status))
			return true;
	}
	return false;
}
...
function promptRuleLabel(rule: PromptProfileRule): string {
	const match =
		rule.match.model ?? rule.match.provider ?? rule.match.family ?? "all";
...
function expectedHashOptions(expectedHash: string | undefined): {
	expectedHash?: string;
} {
...
function findPromptRuleScope(
	...
	if (snapshot.session?.rules.some((rule) => rule.id === ruleId))
		return "session";
	if (snapshot.project?.rules.some((rule) => rule.id === ruleId))
		return "project";
...
async function waitForShutdownCancellation(
