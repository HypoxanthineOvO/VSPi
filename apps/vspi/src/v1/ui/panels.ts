...
} from "../tools/capability-catalog.js";
...
type SkillRow =
	...
	| { kind: "issue"; issue: SkillCatalogIssue };
...
function agentRunSymbol(run: AgentRunSnapshot, theme: VspiTheme): string {
	if (run.status === "success") return theme.success("✓");
	if (
		run.status === "error" ||
		...
		return theme.error("×");
	if (run.status === "cancelled" || run.status === "killed")
		return theme.muted("−");
	if (run.status === "queued") return theme.muted("○");
	return theme.focus("●");
}

function taskDashboardSymbol(
	...
	if (
		...
		return theme.error("×");
	if (item.status === "killed") return theme.muted("−");
	return theme.focus("●");
}

function taskDashboardElapsed(item: TaskDashboardItem): string {
	const seconds = Math.max(
		0,
		Math.round(((item.endedAt ?? Date.now()) - item.startedAt) / 1_000),
	);
	if (seconds < 60) return `${String(seconds)}s`;
	return `${String(Math.floor(seconds / 60))}m${String(seconds % 60).padStart(2, "0")}s`;
}

function compactAgentProcessText(text: string, limit = 160): string {
...
function visibleAgentHistoryBlocks(
	history: readonly AgentConversationBlock[],
): AgentConversationBlock[] {
	const blocks: AgentConversationBlock[] = [];
	const tools = new Map<string, AgentConversationBlock>();
	for (const source of history) {
		if (source.kind === "thinking") continue;
		const block = { ...source };
		if (block.kind === "tool" && block.toolCallId) {
			tools.set(block.toolCallId, block);
			blocks.push(block);
		} else if (block.kind === "error" && block.toolCallId) {
...
function liveAgentProcessBlocks(
	activities: readonly AgentConversationActivity[],
	history: readonly AgentConversationBlock[],
): AgentConversationBlock[] {
	const historyText = history
		.filter((block) => block.kind === "commentary" || block.kind === "final")
		.map((block) => compactAgentProcessText(block.text, Number.POSITIVE_INFINITY))
		.join("\n");
	const historyToolCalls = new Set(
		history.flatMap((block) =>
			block.toolCallId === undefined ? [] : [block.toolCallId],
		),
	);
	const blocks: AgentConversationBlock[] = [];
	const assistantByTurn = new Map<number, AgentConversationBlock>();
	const toolByCall = new Map<string, AgentConversationBlock>();
	for (const activity of activities) {
		if (activity.kind === "assistant") {
			let block = assistantByTurn.get(activity.turnId);
			if (block === undefined) {
				block = {
					id: `live:assistant:${String(activity.turnId)}`,
					kind: "commentary",
				...
				assistantByTurn.set(activity.turnId, block);
				blocks.push(block);
			}
			block.text += activity.delta;
		} else if (
			activity.kind === "tool" &&
			!historyToolCalls.has(activity.toolCallId)
		) {
			let block = toolByCall.get(activity.toolCallId);
			if (block === undefined) {
				block = {
					id: `live:tool:${activity.toolCallId}`,
					kind: "tool",
					sourceRole: "assistant",
					text: activity.state,
					injected: false,
					...
					presentation: activity.state === "error" ? "error" : "message",
				};
				toolByCall.set(activity.toolCallId, block);
				blocks.push(block);
			}
			block.toolName = activity.toolName ?? block.toolName;
			block.presentation = activity.state === "error" ? "error" : "message";
			block.text = activity.text
				? `${activity.state} · ${activity.text}`
				: activity.state;
		}
	}
	return blocks.filter((block) => {
		block.text = compactAgentProcessText(block.text);
		return (
			block.text.length > 0 &&
			(block.kind !== "commentary" || !historyText.includes(block.text))
		);
	});
}
...
function fullscreenScrollbarLabel(
	mode: AppSettings["fullscreenScrollbar"],
): string {
...
function wrapHeadingWithRight(
	title: string,
	right: string,
	width: number,
): string[] {
	if (visibleWidth(title) + visibleWidth(right) + 1 <= width)
		return [alignRight(title, right, width)];
...
function tabLine(
	...
	const line = tabs
		.map((tab, index) =>
			index === selected ? theme.selected(` ${tab} `) : theme.muted(` ${tab} `),
		)
		.join(" ");
...
function clampSelection(state: PanelState, count: number): void {
	state.selected = Math.max(
		0,
		Math.min(state.selected, Math.max(0, count - 1)),
	);
}
...
function commandMatches(query: string): CommandMatch[] {
	...
	const prefixMatches =
		normalized && normalized !== "/" ? matchCommands(query) : [];
	const candidates =
		prefixMatches.length > 0
			? prefixMatches
			: filterCommands(query).map(canonicalMatch);
...
function compareModelGeneration(left: ModelOption, right: ModelOption): number {
	...
	for (let index = 0; index < length; index += 1) {
		const difference =
			(rightGeneration[index] ?? -1) - (leftGeneration[index] ?? -1);
...
function modelGeneration(model: ModelOption): number[] {
	const identity = `${model.id} ${model.label}`
		.toLowerCase()
		.replace(
			/\b(?:19|20)\d{2}(?:[-_/]?(?:0[1-9]|1[0-2])(?:[-_/]?(?:0[1-9]|[12]\d|3[01]))?)?\b/gu,
			" ",
		);
...
function modelCombinedPrice(model: ModelOption): number | undefined {
	if (
		model.price.inputUsdPerMillion === undefined ||
		model.price.outputUsdPerMillion === undefined
	) {
		return undefined;
	}
	return model.price.inputUsdPerMillion + model.price.outputUsdPerMillion;
}
...
export class PanelController {
	...
	private skillSnapshot: SkillCatalogSnapshot = {
		items: [],
		issues: [],
	...
	private agentConversation: AgentConversationPage | undefined;
	private agentConversationLoading = false;
	private agentConversationError: string | undefined;
	private agentActivity: AgentConversationActivity[] = [];
	private agentDetailFollowTail = true;
	private agentDetailResumeFollow = false;
	private agentDetailRenderedRows = 0;
	private agentDetailPrependAnchorRows: number | undefined;
	private agentSelectedRunId: string | undefined;
	private agentFilter: "all" | "active" = "all";
	private agentListScroll = 0;
	private agentPendingStopId: string | undefined;
	private agentNotice: string | undefined;
	...
	hasPlanContent(): boolean {
		return Boolean(
			this.planItems.length > 0 ||
				this.planSnapshot ||
				(this.workflowSnapshot?.status === "ready" &&
					this.workflowSnapshot.delivery),
		);
	}
		...
		if (kind === "agents" || kind === "tasks") {
			const selectedIndex = this.agentSelectedRunId
				? this.agentRuns().findIndex(
						(run) => run.id === this.agentSelectedRunId,
					)
				: -1;
			this.state.selected = Math.max(0, selectedIndex);
		}
		if (kind === "cron") {
			const selectedIndex = this.cronSelectedId
				? this.cronTasks.findIndex((task) => task.id === this.cronSelectedId)
				: -1;
			this.state.selected = Math.max(0, selectedIndex);
		}
		if (kind === "policy")
			this.state.selected = Math.max(
				0,
				POLICY_LEVELS.indexOf(this.policySnapshot.policy),
			);
		if (kind === "effort")
			this.state.selected = Math.max(0, this.effortLevels.indexOf(this.effort));
		if (kind === "question") {
	...
	openQuestions(questions: Question[]): void {
		if (questions.length === 0)
			throw new Error("Question panel requires at least one question");
		this.open("question");
	...
	setSettingsLayers(layers: {
		...
		const preferred =
			this.settingsTab === 1 && layers.project ? layers.project : layers.global;
	...
	setExternalSessions(
		...
		source: ExternalSessionSource = "codex",
	): void {
	...
	confirmModelSelection(
		identity: string | { provider?: string; id: string },
	): void {
		const model =
	...
	confirmModelGroupSelection(groupId: string): void {
		if (this.modelGroups.some((group) => group.id === groupId))
	...
	setProviders(providers: ProviderOption[]): void {
		this.providers = structuredClone(providers).sort(
			(left, right) =>
				providerPriorityIndex(left.id) - providerPriorityIndex(right.id) ||
	...
	setPolicySnapshot(snapshot: PolicySnapshot): void {
		this.policySnapshot = {
			...snapshot,
			sessionAllowlist: [...(snapshot.sessionAllowlist ?? [])],
		};
		if (this.kind === "policy")
			this.state.selected = Math.max(0, POLICY_LEVELS.indexOf(snapshot.policy));
	}

	setAgentSnapshot(snapshot: AgentSnapshot): void {
		const selected =
			this.kind === "agents" || this.kind === "tasks"
				? this.selectedAgentRun()?.id
				: this.agentSelectedRunId;
		this.agentSnapshot = structuredClone(snapshot);
		if (
			this.agentPendingStopId &&
			!this.agentSnapshot.active.some(
				(run) => run.id === this.agentPendingStopId,
			)
		) {
			this.agentPendingStopId = undefined;
		}
		const runs = this.agentRuns();
		const selectedIndex = selected
			? runs.findIndex((run) => run.id === selected)
			: -1;
		if (this.kind === "agents" || this.kind === "tasks") {
			this.state.selected =
				selectedIndex >= 0
					? selectedIndex
					: Math.min(this.state.selected, Math.max(0, runs.length - 1));
			this.agentSelectedRunId = runs[this.state.selected]?.id;
		} else {
			this.agentSelectedRunId =
				runs[selectedIndex >= 0 ? selectedIndex : 0]?.id;
		}
	}

	setTaskSnapshot(snapshot: TaskDashboardSnapshot): void {
		this.taskSnapshot = structuredClone(snapshot);
		if (this.kind === "tasks")
			this.state.selected = Math.min(
				this.state.selected,
				Math.max(0, this.taskItems().length - 1),
			);
	}
	...
	prependAgentConversation(page: AgentConversationPage): void {
		...
		const seen = new Set(
			this.agentConversation.blocks.map((block) => block.id),
		);
		this.agentConversation = {
			...this.agentConversation,
			blocks: [
				...page.blocks.filter((block) => !seen.has(block.id)),
				...this.agentConversation.blocks,
			],
	...
	setAgentConversationLoading(): void {
		this.resetAgentDetailViewport(true);
	}

	setAgentConversationError(error: string): void {
		this.agentConversationLoading = false;
		this.agentConversationError = error;
	}

	appendAgentActivity(activity: AgentConversationActivity): void {
		this.agentActivity.push(structuredClone(activity));
		if (this.agentActivity.length > 100)
			this.agentActivity.splice(0, this.agentActivity.length - 100);
	}

	clearAgentConversation(): void {
		this.resetAgentDetailViewport(false);
	}

	private resetAgentDetailViewport(loading: boolean): void {
		this.agentConversation = undefined;
		this.agentConversationLoading = loading;
		this.agentConversationError = undefined;
		this.agentActivity = [];
		this.agentDetailFollowTail = true;
		this.agentDetailResumeFollow = false;
		this.agentDetailRenderedRows = 0;
		this.agentDetailPrependAnchorRows = undefined;
		this.state.scroll = 0;
	}
	...
	setCronTasks(tasks: readonly CronTask[]): void {
		const selected =
			this.kind === "cron"
				? this.cronTasks[this.state.selected]?.id
				: this.cronSelectedId;
		...
		const selectedIndex = selected
			? this.cronTasks.findIndex((task) => task.id === selected)
			: -1;
		if (this.kind === "cron") {
			this.state.selected =
				selectedIndex >= 0
					...
					: Math.min(
							this.state.selected,
							Math.max(0, this.cronTasks.length - 1),
						);
			this.cronSelectedId = this.cronTasks[this.state.selected]?.id;
		} else {
			this.cronSelectedId =
				this.cronTasks[selectedIndex >= 0 ? selectedIndex : 0]?.id;
		}
	}
	...
	handleInput(data: string): PanelEvent | undefined {
		...
		if (this.kind === "tasks") return this.handleTasks(data);
		...
		if (
			...
			if (
				this.kind === "question" &&
				(this.questionDirectAnswer || this.questionReview)
			) {
				this.questionDirectAnswer = false;
				this.questionReview = false;
				this.questionInput.setValue("");
		...
		if (this.kind === "commands")
			return this.handleCommands(data, interaction.handler);
		...
		if (this.kind === "sessions")
			return this.handleSessions(data, interaction.handler);
		if (this.kind === "externalImport")
			return this.handleExternalImport(data, interaction.handler);
		if (this.kind === "skills")
			return this.handleSkills(data, interaction.handler);
		...
		if (this.kind === "tools") {
			if (interaction.handler === "moveSelection")
				this.move(data, TOOL_CAPABILITIES.length);
			return;
		}
		if (this.kind === "cron") {
			if (interaction.handler === "moveSelection") {
	...
	render(
		...
		let body: string[];
		if (this.kind === "commands")
			[title, body] = ["命令", this.renderCommands(bodyWidth, theme)];
		else if (this.kind === "goal")
			[title, body] = ["Goal", this.renderGoal(bodyWidth, theme)];
		else if (this.kind === "prompt")
			...
			[title, body] = [
				"Provider",
				this.renderProviders(bodyWidth, bodyRows, theme),
			];
		else if (this.kind === "sessions")
			[title, body] = ["Sessions", this.renderSessions(bodyWidth, theme)];
		else if (this.kind === "externalImport")
			[title, body] = [
				"导入会话",
				this.renderExternalImport(bodyWidth, bodyRows, theme),
			];
		else if (this.kind === "skills")
			[title, body] = ["Skills", this.renderSkills(bodyWidth, bodyRows, theme)];
		else if (this.kind === "settings")
			[title, body] = ["Settings", this.renderSettings(bodyWidth, theme)];
		else if (this.kind === "usage")
			[title, body] = ["Usage", this.renderUsage(bodyWidth, usage)];
		else if (this.kind === "theme")
			[title, body] = ["Theme", this.renderTheme(bodyWidth, theme)];
		else if (this.kind === "question")
			[title, body] = ["Question", this.renderQuestion(bodyWidth, theme)];
		else if (this.kind === "approval")
			[title, body] = ["需要批准", this.renderApproval(bodyWidth, theme)];
		else if (this.kind === "effort")
			[title, body] = ["Effort", this.renderEffort(bodyWidth, theme)];
		else if (this.kind === "tools")
			[title, body] = ["Tools", this.renderTools(bodyWidth, theme)];
		else if (this.kind === "agents") {
			const selected = this.selectedAgentRun();
			[title, body] = [
				this.agentView === "detail" && selected
					? `${selected.codename ?? selected.agentId} · ${selected.agentId}`
					: "Agents",
				this.renderAgents(bodyWidth, bodyRows, theme),
			];
		} else if (this.kind === "tasks") {
			[title, body] = ["Tasks", this.renderTasks(bodyWidth, theme)];
		} else if (this.kind === "cron")
			[title, body] = ["Cron", this.renderCron(bodyWidth, theme)];
		else if (this.kind === "policy")
			[title, body] = ["Policy", this.renderPolicy(bodyWidth, theme)];
		else
			[title, body] = ["Plan", this.renderPlan(bodyWidth, theme, planFocused)];

		if (
			this.kind === "plan" &&
			this.planPanelCollapsed &&
			this.planItems.length > 0
		) {
			return [
				padLine(
					`${theme.focus("Plan")} · ${theme.blue(theme.bold(this.planDisplayTitle()))}`,
					width,
				),
			];
		}
		...
		let footer =
			this.kind === "question" ? this.renderQuestionFooter(theme) : undefined;
		let rightTitle: string | undefined;
		const questionFooterGap =
			this.kind === "question" &&
			(this.questionSelectionRange !== undefined || this.questionReview);
		const visibleBodyRows = questionFooterGap
			? Math.max(1, bodyRows - 1)
			: bodyRows;
		if (this.kind === "agents" && this.agentView === "detail") {
			const maxScroll = Math.max(0, body.length - visibleBodyRows);
			if (
				!this.agentDetailFollowTail &&
				this.agentDetailPrependAnchorRows !== undefined
			) {
				this.state.scroll += Math.max(
					0,
					body.length - this.agentDetailPrependAnchorRows,
				);
			}
			this.agentDetailPrependAnchorRows = undefined;
			this.agentDetailRenderedRows = body.length;
			if (this.agentDetailFollowTail) this.state.scroll = maxScroll;
			else {
				this.state.scroll = Math.max(0, Math.min(this.state.scroll, maxScroll));
				if (this.agentDetailResumeFollow && this.state.scroll >= maxScroll) {
					this.agentDetailFollowTail = true;
		...
		if (body.length > visibleBodyRows) {
			if (this.kind === "question" && this.questionReview) {
				this.state.scroll = Math.max(
					0,
					Math.min(this.state.scroll, body.length - visibleBodyRows),
				);
				const total = body.length;
				body = body.slice(
					this.state.scroll,
					this.state.scroll + visibleBodyRows,
				);
				...
				const pinnedRows = Math.min(
					this.questionPinnedRows,
					Math.max(0, visibleBodyRows - 2),
				);
				...
				const selectionStart = Math.max(
					0,
					this.questionSelectionRange[0] - pinnedRows,
				);
				const selectionEnd = Math.max(
					selectionStart,
					this.questionSelectionRange[1] - pinnedRows,
				);
				this.state.scroll = Math.max(
					0,
					Math.min(
						this.state.scroll,
						Math.max(0, scrollable.length - scrollRows),
					),
				);
				...
				if (selectionRows > scrollRows) {
					this.state.scroll = Math.max(
						...
						Math.min(
							this.state.scroll,
							Math.max(selectionStart, selectionEnd - scrollRows + 1),
						),
					);
				} else {
					if (selectionStart < this.state.scroll)
						this.state.scroll = selectionStart;
					if (selectionEnd >= this.state.scroll + scrollRows)
						this.state.scroll = selectionEnd - scrollRows + 1;
				}
				const visible = scrollable.slice(
					this.state.scroll,
					this.state.scroll + scrollRows,
				);
				body = [...pinned, ...visible];
				...
				if (this.kind === "agents" && this.agentView === "detail") {
					...
					selectionStart = Math.max(
						0,
						Math.min(
							highlightedRows[0] ?? this.state.selected,
							body.length - 1,
						),
					);
				...
				if (
					selectionEnd - selectionStart < visibleBodyRows &&
					selectionEnd >= this.state.scroll + visibleBodyRows
				) {
	...
	renderSessionsSurface(
		...
		this.state.scroll = Math.max(
			0,
			Math.min(this.state.scroll, Math.max(0, body.length - bodyRows)),
		);
	...
	renderHint(width: number, theme: VspiTheme): string {
		...
		const contextualHint =
			this.kind === "agents"
				? this.agentPendingStopId
					? `停止 ${this.agentPendingStopId}？ Y 确认  N/Esc 取消`
					: this.agentView === "detail"
						? "←→ 切换 Agent  ↑↓/PgUp/PgDn 滚动  Esc 返回"
						: "↑↓ 选择  Enter 详情  S 停止  R 刷新  Tab 筛选  Esc 关闭"
				: this.kind === "tasks"
					? this.agentPendingStopId
						? `停止 ${this.agentPendingStopId}？ Y 确认  N/Esc 取消`
						: "↑↓ 选择  S 停止  R 刷新  Esc 关闭"
	...
	private questionHintText(): string {
		return this.questionHint(
			renderInteractionHint("panel", "question", this.interactionState()),
		);
	}
	...
	private questionHint(hint: string): string {
		...
		if (this.questionDirectAnswer) {
			return hint
				.replace("  Shift+S 跳过", "")
				.replace("Enter 确认", "Enter 确认  Esc 返回选项");
		}
		if (question.kind === "freeText") return hint.replace("  Shift+S 跳过", "");
		let adjusted = hint;
		if (question.kind === "ranking")
			adjusted = adjusted.replace("Enter 确认", "Tab 直接回答  Enter 确认");
		if (question.kind !== "multiChoice")
			adjusted = adjusted.replace("Space 多选  ", "");
		return adjusted;
	}
	...
	acceptsInput(data: string): boolean {
		...
		if (
			this.kind === "commands" &&
			panelKey(data, Key.enter) &&
			/\s/.test(this.commandQuery.trim())
		) {
			return false;
		}
		if (
			...
			[
				...
				Key.left,
				Key.right,
			].some((key) => panelKey(data, key))
		)
			return true;
		return (
			matchingInteraction("panel", this.kind, data, this.interactionState()) !==
			undefined
		);
	}
	...
	private interactionState(): InteractionState {
		...
		if (this.kind === "plan")
			...
			state.hasItems = this.sessions.length > 0;
		else if (this.kind === "externalImport")
			state.hasItems = this.filteredExternalSessions().length > 0;
		else if (this.kind === "skills") {
			...
			if (selected)
				state.commandAvailable =
					getActionDefinition(selected.command)?.availability !== "disabled";
		} else if (this.kind === "providers") {
			state.providerEditing = this.providerEditing;
			state.providerActionMenu = this.providerActionMenu;
			if (this.providerEditing) {
				state.providerField =
					this.providerField === 0 ? 0 : this.providerField === 1 ? 1 : 2;
				const text =
					this.providerField === 0
						? this.providerDraft.label
						: this.providerDraft.baseUrl;
	...
	private handleModels(data: string): PanelEvent | undefined {
		...
		if (this.modelTab === 0) {
			const next = this.editSharedTextField(
				"model-search",
				this.modelSearch,
				data,
				500,
	...
	private handleProviders(data: string): PanelEvent | undefined {
		if (this.providerEditing) {
			...
			if (
				this.providerField === 2 &&
				(matchesKey(data, Key.left) || matchesKey(data, Key.right))
			) {
				this.providerDraft.protocol =
					this.providerDraft.protocol === "Responses"
						? "OpenAI compatible"
						: "Responses";
				return;
			}
			const key =
				this.providerField === 0
					? "label"
					: this.providerField === 1
						? "baseUrl"
						: undefined;
			if (key) {
				this.providerDraft[key] = this.editSharedTextField(
					`provider-${key}`,
					this.providerDraft[key],
					data,
					2_000,
				);
			}
			if (matchesKey(data, Key.ctrl("s"))) {
				...
				if (!provider)
					return {
						type: "notice",
						text: "Provider identity 已失效，拒绝保存",
						tone: "error",
					};
				this.providerEditing = false;
				return {
		...
		if (this.providerActionMenu) {
			...
			if (!provider || !action) {
				...
				return {
					type: "notice",
					text: "Provider identity 已失效，操作已取消",
					tone: "error",
				};
			}
	...
	private handleSessions(
		...
		if (handler === "moveSelection" && this.move(data, this.sessions.length))
			return;
	...
	private handleExternalImport(
		...
		if (handler === "switchImportSource") {
			this.externalImportSource =
				this.externalImportSource === "codex" ? "claude" : "codex";
			this.state.selected = 0;
			this.state.scroll = 0;
			return;
		}
	...
	private handleSkills(data: string, handler: string): PanelEvent | undefined {
		...
		if (handler === "switchSkillScope") {
			if (this.skillSnapshot.projectTrusted)
				this.skillScope = this.skillScope === "user" ? "project" : "user";
			return;
		}
		if (handler === "editSkillText") {
			const current = this.skillAdding ? this.skillAddText : this.skillSearch;
			const next = this.editSharedTextField(
				this.skillAdding ? "skill-add" : "skill-search",
				current,
				data,
				2_000,
			);
		...
		if (handler === "switchSkillTab") {
			...
			this.skillTab =
				tabs[(tabs.indexOf(this.skillTab) + 1) % tabs.length] ?? "enabled";
			this.state.selected = 0;
			this.state.scroll = 0;
			this.skillViewing = false;
			return;
		...
		if (handler === "toggleSkill")
			return {
				type: "skillToggle",
				skill: row.item,
				enabled: !row.item.enabled,
			};
		if (handler === "updateSkill")
			return { type: "skillUpdate", skill: row.item };
		if (handler === "removeSkill")
			return { type: "skillRemove", skill: row.item };
		return undefined;
	}
	...
	private handleSettings(data: string): PanelEvent | undefined {
		if (this.settingsEndpointEditing) {
			...
			if (panelKey(data, Key.enter)) {
				...
				this.settingsDirty ||=
					endpoint !== this.settings.thinkingTranslationEndpoint;
				this.settings.thinkingTranslationEndpoint = endpoint;
				this.settingsEndpointEditing = false;
				this.settingsEndpointInput = "";
				return;
			}
			this.settingsEndpointInput = this.editSharedTextField(
				"settings-endpoint",
				this.settingsEndpointInput,
				data,
				500,
		...
		if (
			...
			const next =
				nextTab === 0
					...
					: this.settingsLayers.project;
			if (!next)
				return {
					type: "notice",
					text: "项目未授予 trust，无法编辑项目设置",
					tone: "warning",
				};
		...
		if (matchesKey(data, Key.enter) || matchesKey(data, Key.space)) {
			...
			if (row.key === "theme") {
				...
				this.settings.theme =
					themes[(themes.indexOf(this.settings.theme) + 1) % themes.length] ??
					"VSPi Dark";
			...
			if (row.key === "tuiMode") {
				this.settings.tuiMode =
					this.settings.tuiMode === "fullscreen" ? "regular" : "fullscreen";
				this.settingsDirty = true;
				return;
			}
			if (row.key === "fullscreenScrollbar") {
				const scrollbars: AppSettings["fullscreenScrollbar"][] = [
					"auto",
					"always",
					"hidden",
				];
				this.settings.fullscreenScrollbar =
					scrollbars[
						(scrollbars.indexOf(this.settings.fullscreenScrollbar) + 1) %
							scrollbars.length
					] ?? "auto";
				this.settingsDirty = true;
				return;
			...
			if (row.key === "mermaidRendering") {
				const modes: AppSettings["mermaidRendering"][] = [
					"off",
					"final",
					"streaming",
				];
				this.settings.mermaidRendering =
					modes[
						(modes.indexOf(this.settings.mermaidRendering) + 1) % modes.length
					] ?? "final";
				this.settingsDirty = true;
				return;
			}
			if (row.key === "workingStyle") {
				...
				this.settings.workingStyle =
					styles[
						(styles.indexOf(this.settings.workingStyle) + 1) % styles.length
					] ?? 3;
				this.settingsDirty = true;
				return;
			}
			if (row.key === "thinkingDisplay") {
				const modes: AppSettings["thinkingDisplay"][] = [
					"hidden",
					"collapsed",
					"expanded",
				];
				this.settings.thinkingDisplay =
					modes[
						(modes.indexOf(this.settings.thinkingDisplay) + 1) % modes.length
					] ?? "collapsed";
				this.settingsDirty = true;
				return;
			}
	...
	private handleApproval(data: string): PanelEvent | undefined {
		if (this.approvalReasonEditing) {
			if (matchesKey(data, Key.enter)) {
				...
				return {
					type: "approval",
					response: { type: "deny", ...(reason ? { reason } : {}) },
				};
			}
			this.approvalReason = this.editSharedTextField(
				"approval-reason",
				this.approvalReason,
				data,
				500,
		...
		if (this.state.selected === 0)
			return { type: "approval", response: { type: "allow-once" } };
		...
		if (request?.requiredPolicy && this.state.selected === 2) {
			return {
				type: "approval",
				response: { type: "elevate", level: request.requiredPolicy },
			};
		}
		const denialIndex = request?.requiredPolicy ? 3 : 2;
		if (this.state.selected === denialIndex)
			return { type: "approval", response: { type: "deny" } };
	...
	private handlePlan(data: string, handler: string): PanelEvent | undefined {
		...
		if (this.planNextActionEditing) {
			...
			this.planNextActionInput = this.editSharedTextField(
				"plan-next-action",
				this.planNextActionInput,
				data,
				4_000,
		...
		if (this.planActionMenu) {
			if (panelKey(data, Key.up))
				this.planActionIndex = Math.max(0, this.planActionIndex - 1);
			else if (panelKey(data, Key.down))
				this.planActionIndex = Math.min(2, this.planActionIndex + 1);
				...
				if (this.planActionIndex === 0) {
					...
					return {
						...
						operation: {
							kind: "status",
							itemId: item.id,
							status: nextPlanStatus(item.id, this.planSnapshot),
						},
					};
				}
	...
	private handlePrompt(data: string): PanelEvent | undefined {
		if (this.promptImportEditing) {
			...
			this.promptImportPath = this.editSharedTextField(
				"prompt-import-path",
				this.promptImportPath,
				data,
				4_096,
		...
		if (data === Key.enter && profile)
			return { type: "promptPin", profileId: profile.id };
		if (data === "o") return { type: "promptOff" };
		if (data === "f" && profile)
			return { type: "promptFork", profileId: profile.id };
		if (data === "i") {
		...
		if (data === "e" && profile)
			return { type: "promptExport", profileId: profile.id };
		if (data === "t") {
	...
	private handleQuestion(data: string): PanelEvent | undefined {
		if (this.questionReview) {
			...
			else if (panelKey(data, Key.pageUp))
		...
		if (this.questionDirectAnswer || question.kind === "freeText") {
			...
			if (Array.from(this.questionInput.getValue()).length > 2_000)
				this.questionInput.setValue(previous);
			return;
		}
		if (panelKey(data, Key.left) || panelKey(data, Key.right)) {
			...
			this.questionIndex = Math.max(
				0,
				Math.min(this.questions.length - 1, this.questionIndex + offset),
			);
			this.questionDirectAnswer = false;
			this.questionInput.setValue("");
			this.state.selected = 0;
		...
		const rankingUp =
			panelKey(data, Key.ctrl("up")) || panelKey(data, Key.alt("up"));
		const rankingDown =
			panelKey(data, Key.ctrl("down")) || panelKey(data, Key.alt("down"));
	...
	private renderCommands(width: number, theme: VspiTheme): string[] {
		const matches = commandMatches(this.commandQuery);
		if (matches.length === 0)
			return [theme.muted(padLine("没有匹配的命令", width))];
		const lines: string[] = [];
	...
	private renderCommandRow(
		...
		const source =
			match.source === BUILTIN_COMMAND_SOURCE ? "Built-in" : match.source;
		...
		if (width < 58) {
			...
			return [
				selectedLine(identity, selected, width, theme),
				selectedLine(detail, selected, width, theme),
			];
		}
		...
		const descriptionWidth = Math.max(
			1,
			available - identityWidth - sourceWidth,
		);
	...
	private filteredModels(): ModelOption[] {
		...
		const models = this.models
			...
			.sort((left, right) => {
				...
				const rightPrice = modelCombinedPrice(right);
				if (leftPrice === undefined && rightPrice !== undefined) return 1;
				if (leftPrice !== undefined && rightPrice === undefined) return -1;
				const price = (rightPrice ?? 0) - (leftPrice ?? 0);
	...
	private renderNarrowModels(
		...
		if (this.modelNarrowDetail) {
			const details =
				this.modelTab === 0
					? this.modelDetailRows(rowCount, width, theme)
					: this.modelGroupDetailRows(rowCount, theme);
			return Array.from({ length: rowCount }, (_, index) =>
				padLine(details[index] ?? "", width),
			);
		}
		return this.modelListRows(rowCount, width, theme);
	}

	private modelListRows(
		rowCount: number,
		width: number,
		theme: VspiTheme,
	): string[] {
		if (this.modelTab === 1) {
			const start = Math.max(
				0,
				Math.min(
					this.state.selected - Math.floor(rowCount / 2),
					this.modelGroups.length - rowCount,
				),
			);
			return Array.from({ length: rowCount }, (_, row) => {
				...
				const check =
					group.id === this.selectedGroupId ? theme.success("✓ ") : "  ";
		...
		const selectedRow = Math.max(
			...
			entries.findIndex(
				(entry) =>
					entry.type === "model" && entry.modelIndex === this.state.selected,
			),
		);
		const start = Math.max(
			0,
			Math.min(
				selectedRow - Math.floor(rowCount / 2),
		...
		if (first?.type === "model") {
			const providerRow = entries.findLastIndex(
				(entry, index) =>
					index < start &&
					entry.type === "provider" &&
					entry.key === first.providerKey,
			);
			if (providerRow >= 0)
				visible = [
					entries[providerRow] as ModelListEntry,
					...entries.slice(start, start + rowCount - 1),
				];
		}
		...
		return Array.from({ length: rowCount }, (_, row) => {
			...
			if (entry.type === "provider") {
				return alignRight(
					theme.bold(theme.muted(`  ${entry.label}`)),
					theme.muted(String(entry.count)),
					width,
				);
			}
			...
			const marker = selected ? theme.focus("› ") : "  ";
			const check =
				modelKey(entry.model) === this.selectedModelKey
					? theme.success("✓ ")
					: "  ";
		...
		models.forEach((model, modelIndex) => {
			if (model.brand !== providerKey) {
				...
				entries.push({
					...
					key: providerKey,
					label: providerKey,
					count: counts.get(providerKey) ?? 0,
				});
			}
	...
	private modelDetailRows(
		...
		const input =
			model.price.inputUsdPerMillion === undefined
				? undefined
				: model.price.inputUsdPerMillion * FX.fxRate;
		const output =
			model.price.outputUsdPerMillion === undefined
				? undefined
				: model.price.outputUsdPerMillion * FX.fxRate;
		const cacheRead =
			model.price.cacheReadUsdPerMillion === undefined
				? undefined
				: model.price.cacheReadUsdPerMillion * FX.fxRate;
		const cacheWrite =
			model.price.cacheWriteUsdPerMillion === undefined
				? undefined
		...
		const basePrice =
			input === undefined && output === undefined
				? theme.muted("价格      未提供")
				: `${theme.muted(model.price.source === "official" ? "官方参考  " : "渠道价    ")}${model.price.contextTiers?.length ? theme.muted("基础  ") : ""}${theme.warning("输入 ¥")}${input?.toFixed(2) ?? "—"} / 百万  ${theme.warning("输出 ¥")}${output?.toFixed(2) ?? "—"} / 百万`;
		const tierPrices = (model.price.contextTiers ?? []).map(
			(tier) =>
				`${theme.muted(`>${formatAgentTokens(tier.contextTokensAbove)}  `)}${theme.warning("输入 ¥")}${(tier.inputUsdPerMillion * FX.fxRate).toFixed(2)}  ${theme.warning("输出 ¥")}${(tier.outputUsdPerMillion * FX.fxRate).toFixed(2)} / 百万`,
		);
		const cachePrice =
			cacheRead === undefined && cacheWrite === undefined
				? undefined
				: `${theme.muted("缓存      ")}${theme.warning("读取 ¥")}${cacheRead?.toFixed(2) ?? "—"} / 百万  ${theme.warning("写入 ¥")}${cacheWrite?.toFixed(2) ?? "—"} / 百万`;
		...
		return details.length <= rowCount
			? details
			: [...details.slice(0, rowCount - prices.length), ...prices].slice(
					0,
					rowCount,
				);
	}
	...
	private modelGroupDetailRows(rowCount: number, theme: VspiTheme): string[] {
		...
		for (const role of group.roles) {
			...
			rows.push(
				`${theme.muted(`${role.role}  `)}${model?.label ?? role.modelId} · Effort ${effortLabel(role.effort)}`,
			);
		}
	...
	private renderProviders(
		...
		if (this.providerEditing) {
			...
			return fields.map(([label, value], index) =>
				selectedLine(
					`${theme.muted(`${label}  `)}${value}`,
					index === this.providerField,
					width,
					theme,
				),
			);
		...
		if (this.providers.length === 0)
			return [theme.muted(padLine("没有可用 Provider", width))];
	...
	private providerListRows(
		...
		const start = Math.max(
			...
			Math.min(
				this.state.selected - Math.floor(rowCount / 2),
				Math.max(0, this.providers.length - rowCount),
			),
		);
		return Array.from({ length: rowCount }, (_, row) => {
			const index = start + row;
			...
			return selectedLine(
				alignRight(
					`${provider.label}  ${theme.muted(provider.protocol)}`,
					status,
					Math.max(1, width - 2),
				),
				index === this.state.selected,
	...
	private renderSessions(width: number, theme: VspiTheme): string[] {
		if (this.sessions.length === 0)
			return [theme.muted(padLine("暂无会话", width))];
		return this.sessions.flatMap((session, index) => {
			...
			const line = alignRight(
				`${marker}${selected ? theme.focus(theme.bold(label)) : label}`,
				status,
				width,
			);
	...
	private renderExternalImport(
		width: number,
		rows: number,
		theme: VspiTheme,
	): string[] {
		const codexCount = this.externalSessions.filter(
			(session) => session.source === "codex",
		).length;
		const claudeCount = this.externalSessions.filter(
		...
		const search = padLine(
			`  ${theme.muted("搜索  ")}${query}${theme.inverse(" ")}`,
			width,
		);
		...
		if (width < 58)
			return [
				tabs,
				search,
				...this.externalImportList(sessions, width, contentRows, theme),
			];

		const listWidth = Math.max(28, Math.floor(width * 0.57));
		const detailWidth = Math.max(1, width - listWidth - 1);
		const list = this.externalImportList(
			sessions,
			listWidth,
			contentRows,
			theme,
		);
		const detail = this.externalImportDetail(
			sessions[this.state.selected],
			detailWidth,
			contentRows,
			theme,
		);
	...
	private renderSkills(
		...
		if (this.skillViewing && width < 58) {
			return this.skillDetail(
				this.skillRows()[this.state.selected],
				width,
				rows,
				theme,
			);
		}
		const enabledCount = this.skillSnapshot.items.filter(
			(item) => item.enabled,
		).length;
	...
	private renderSkillAdd(
		width: number,
		rows: number,
		theme: VspiTheme,
	): string[] {
		const tabs = [
			this.skillAddMode === "source"
				? theme.selected(" URL / npm ")
				: theme.muted(" URL / npm "),
			this.skillAddMode === "agent"
				? theme.selected(" 让 Agent 帮我找 ")
				: theme.muted(" 让 Agent 帮我找 "),
		].join(" ");
		...
		const value =
			...
			theme.muted(
				this.skillAddMode === "source"
					? "Git URL 或 npm:package"
					: "描述 Skill 用途",
			);
		const global =
			this.skillScope === "user"
				? theme.selected(" Global ")
				: theme.muted(" Global ");
		const projectLabel = this.skillSnapshot.projectTrusted
			? " Project "
			: " Project · Untrusted ";
		const project =
			this.skillScope === "project"
				? theme.selected(projectLabel)
				: theme.muted(projectLabel);
		const body = [
			padLine(tabs, width),
			padLine("", width),
		...
		return Array.from(
			{ length: rows },
			(_, index) => body[index] ?? padLine("", width),
		);
	}
	...
	private skillList(
		...
		if (rows.length === 0)
			return [
				theme.muted(
					padLine(
						this.skillTab === "issues" ? "没有 Skill 问题" : "没有匹配的 Skill",
						width,
					),
				),
			];
		const start = Math.max(
			0,
			Math.min(
				this.state.selected - Math.floor(count / 2),
		...
		return Array.from({ length: count }, (_, offset) => {
			...
			const scope =
				row.item.scope === "project"
					? "Project"
					: row.item.scope === "user"
						? "Global"
						: row.item.sourceLabel;
			const source =
				row.item.scope === "external"
					? scope
					: `${scope} · ${row.item.sourceLabel}`;
			return selectedLine(
	...
	private skillRows(): SkillRow[] {
		...
		return this.skillSnapshot.items
			.filter((item) =>
				this.skillTab === "enabled" ? item.enabled : !item.enabled,
			)
			.filter(
				(item) =>
					!query ||
	...
	private externalImportList(
		...
		if (sessions.length === 0)
			return [theme.muted(padLine("没有匹配的历史会话", width))];
		const start = Math.max(
			0,
			Math.min(
				this.state.selected - Math.floor(rows / 2),
		...
		return Array.from({ length: rows }, (_, row) => {
			...
			return selectedLine(
				alignRight(
					session.title,
					formatExternalDate(session.updatedAt),
					Math.max(1, width - 2),
				),
				index === this.state.selected,
				width,
	...
	private settingRows(): Array<{
		...
		return [
			...
			{
				group: "外观",
				label: `减少动效  ${this.settings.reducedMotion ? "开" : "关"}`,
				key: "reducedMotion",
			},
			{
				group: "外观",
			...
			{
				group: "Transcript",
				label: `代码自动换行  ${this.settings.wrapCode ? "开" : "关"}`,
				key: "wrapCode",
			},
			{
				group: "Transcript",
			...
			{
				group: "会话",
				label: `退出时 AI 总结标题  ${this.settings.summarizeSessionTitleOnExit ? "开" : "关"}`,
				key: "summarizeSessionTitleOnExit",
			},
			{
				group: "终端",
	...
	private renderSettings(width: number, theme: VspiTheme): string[] {
		...
		const dirty = this.settingsDirty
			...
			: theme.muted("已同步");
		const lines = [
			tabLine(["全局", projectLabel], this.settingsTab, width, theme),
			alignRight("", dirty, width),
		];
		...
		this.settingRows().forEach((row, index) => {
			...
			const label =
				row.key === "thinkingTranslationEndpoint" &&
				this.settingsEndpointEditing
					? `思考翻译服务  ${this.settingsEndpointInput}${theme.inverse(" ")}`
					: row.label;
			lines.push(
				selectedLine(label, index === this.state.selected, width, theme),
			);
		});
	...
	private renderUsage(width: number, usage: UsageSnapshot): string[] {
		...
		const lines = [
			...
			alignRight("输出 ↓", usage.outputTokens.toLocaleString("zh-CN"), width),
			alignRight("Cache Read", tokens(usage.cacheReadTokens), width),
			alignRight("Cache Write", tokens(usage.cacheWriteTokens), width),
			alignRight(
				"当前 Turn 命中率",
				percent(usage.recentCacheHitPercent),
				width,
			),
			alignRight(
				"Session 命中率",
		...
		if (usage.officialCostCny !== null)
			lines.push(
				alignRight("官方成本", `¥${usage.officialCostCny.toFixed(2)}`, width),
			);
		if (usage.providerBilledCny !== null)
			lines.push(
				alignRight(
					"Provider 账单",
					`¥${usage.providerBilledCny.toFixed(2)}`,
					width,
				),
			);
		return lines;
	...
	private renderPolicy(width: number, theme: VspiTheme): string[] {
		...
		if (selected === "Auto") {
			rows.push(
				theme.warning(
					padLine("Auto · Host：本会话所有工具调用都不再询问", width),
				),
			);
		} else if (this.policySnapshot.recovery) {
			rows.push(
				theme.warning(
					padLine("Recovery 强制 Standard · Host，拒绝切换", width),
				),
			);
		} else {
			rows.push(
		...
		if ((this.policySnapshot.sessionAllowlist?.length ?? 0) > 0) {
			rows.push(
				theme.muted(
					padLine(
						`本会话已允许  ${this.policySnapshot.sessionAllowlist.join("、")}`,
						width,
					),
				),
			);
	...
	private renderTools(width: number, theme: VspiTheme): string[] {
		...
		return TOOL_CAPABILITIES.flatMap((capability, index) => {
			...
			const detail = padLine(
				`    ${capability.route} · ${capability.boundary}`,
				width,
			);
			return [
				selectedLine(header, selected, width, theme),
				selected ? theme.selected(detail) : theme.muted(detail),
			];
		});
	}
	...
	private agentRuns(): AgentRunSnapshot[] {
		...
		for (const run of this.agentSnapshot.recent)
			if (!byId.has(run.id)) byId.set(run.id, run);
		return [...byId.values()]
			.filter(
				(run) => this.agentFilter === "all" || !agentRunTerminal(run.status),
			)
			.sort((left, right) => {
				...
				const rightAt =
					Date.parse(right.finishedAt ?? right.startedAt ?? "") || 0;
	...
	private handleAgents(data: string): PanelEvent | undefined {
		if (this.agentPendingStopId) {
			const taskId = this.agentPendingStopId;
			this.agentPendingStopId = undefined;
			if (data === "y" || data === "Y") return { type: "agentStop", taskId };
			return;
		}
		if (panelKey(data, Key.escape)) {
			if (this.agentView === "detail") {
				this.agentView = "list";
				this.clearAgentConversation();
				this.state.scroll = 0;
				return { type: "agentConversationClose" };
			}
			this.close();
			return { type: "close" };
		}
		...
		if (this.agentView === "detail") {
			if (panelKey(data, Key.left) || panelKey(data, Key.right)) {
				const offset = panelKey(data, Key.left) ? -1 : 1;
				const nextIndex = this.state.selected + offset;
				if (nextIndex < 0 || nextIndex >= runs.length) return;
				this.state.selected = nextIndex;
				const selected = this.selectedAgentRun();
			...
			if (panelKey(data, Key.up)) {
				this.agentDetailFollowTail = false;
				this.agentDetailResumeFollow = false;
				this.state.scroll = Math.max(0, this.state.scroll - 1);
			} else if (panelKey(data, Key.down)) {
				this.agentDetailResumeFollow = true;
				this.state.scroll += 1;
			} else if (panelKey(data, Key.pageUp)) {
				if (
					this.state.scroll === 0 &&
					this.agentConversation?.nextCursor !== undefined
				) {
					const selected = this.selectedAgentRun();
					if (selected !== undefined) {
						const cursor = this.agentConversation.nextCursor;
						this.agentConversation = {
							...this.agentConversation,
			...
			} else if (panelKey(data, Key.pageDown)) {
				this.agentDetailResumeFollow = true;
				this.state.scroll += page;
			}
			return;
		}
		if (panelKey(data, Key.tab)) {
			const selectedId = this.selectedAgentRun()?.id;
			this.agentFilter = this.agentFilter === "all" ? "active" : "all";
			const filtered = this.agentRuns();
			const selectedIndex = selectedId
				? filtered.findIndex((run) => run.id === selectedId)
				: -1;
			this.state.selected = selectedIndex >= 0 ? selectedIndex : 0;
			this.agentSelectedRunId = filtered[this.state.selected]?.id;
			this.agentListScroll = 0;
			this.agentNotice = undefined;
			const selected = this.selectedAgentRun();
		...
		if (data === "r" || data === "R") {
			this.agentNotice = "已刷新";
			return { type: "agentRefresh" };
		}
		if (data === "s" || data === "S") {
			const selected = this.selectedAgentRun();
			if (!selected) return;
			if (agentRunTerminal(selected.status)) {
				this.agentNotice = `${selected.id} 已结束，无需停止`;
				return;
			}
			this.agentPendingStopId = selected.id;
			this.agentNotice = undefined;
			return;
		}
		...
		if (panelKey(data, Key.left)) {
			...
			const index = parentId
				? runs.findIndex((run) => run.id === parentId)
				: -1;
		...
		if (panelKey(data, Key.right)) {
			...
			const index = selectedId
				? runs.findIndex((run) => run.parentId === selectedId)
				: -1;
			if (index >= 0) this.state.selected = index;
			this.agentSelectedRunId = this.selectedAgentRun()?.id;
		}
	...
	private handleTasks(data: string): PanelEvent | undefined {
		if (this.agentPendingStopId) {
			const taskId = this.agentPendingStopId;
			this.agentPendingStopId = undefined;
			if (data === "y" || data === "Y") return { type: "agentStop", taskId };
		...
		if (panelKey(data, Key.up) || panelKey(data, Key.down)) {
			this.move(data, items.length);
			return;
		}
		if (data === "r" || data === "R") return { type: "agentRefresh" };
		if (data === "s" || data === "S") {
			const selected = items[this.state.selected];
			if (selected?.kind === "agent" && selected.status === "running")
				this.agentPendingStopId = selected.taskId;
		}
		return;
	}
	...
	private renderGoal(width: number, theme: VspiTheme): string[] {
		...
		if (!goal)
			return [theme.muted(padLine("当前 Session 没有绑定 Goal", width))];
		...
		const rows = [
			alignRight(
				theme.bold(theme.focus(state)),
				theme.muted(`r${goal.revision}`),
				width,
			),
			...
			...(marker?.currentItem
				? [truncateToWidth(`当前  ${marker.currentItem}`, width)]
				: []),
			...(marker?.nextItem
				? [truncateToWidth(`下一步  ${marker.nextItem}`, width)]
				: []),
			...(marker?.completedWork.length
				? [
						theme.muted("最近完成"),
						...marker.completedWork.flatMap((item) =>
							wrapTextWithAnsi(`  ${item}`, width),
						),
					]
				: []),
			...
			...(goal.blocker
				...
				: []),
			...(goal.stateReason
				? [theme.muted(truncateToWidth(`状态原因  ${goal.stateReason}`, width))]
				: []),
	...
	private renderAgents(
		...
		if (this.agentView === "detail") {
			...
			return this.renderAgentDetail(selected, width, theme);
		}
		const runs = this.agentRuns();
		const allRuns = [
			...this.agentSnapshot.active,
			...this.agentSnapshot.recent.filter(
				(run) =>
					!this.agentSnapshot.active.some((active) => active.id === run.id),
			),
		];
		const active = allRuns.filter(
			(run) => !agentRunTerminal(run.status),
		).length;
		const completed = allRuns.filter((run) => run.status === "success").length;
		const interrupted = allRuns.filter(
			(run) => agentRunTerminal(run.status) && run.status !== "success",
		).length;
		const filter =
		...
		const activeFilter =
			this.agentFilter === "active"
				...
				: theme.muted(" Active ");
		const counts = theme.muted(
			`${active} active · ${completed} completed · ${interrupted} interrupted · ${allRuns.length} total`,
		);
		const header = padLine(
			truncateToWidth(`${filter} ${activeFilter}  ${counts}`, width),
			width,
		);
		if (this.agentView === "list" && width >= 72 && rows >= 8) {
			return [header, ...this.renderAgentColumns(runs, width, rows - 1, theme)];
		}
		const lines: string[] = [header];
		if (this.agentSnapshot.diagnostic)
			lines.push(theme.warning(this.agentSnapshot.diagnostic));
	...
	private renderTasks(width: number, theme: VspiTheme): string[] {
		...
		if (this.taskSnapshot.agents.length === 0)
			lines.push(theme.muted("  No Agent jobs"));
		for (const item of this.taskSnapshot.agents) {
			const index = items.indexOf(item);
			const pointer = index === this.state.selected ? "›" : " ";
			const row = `${pointer} ${taskDashboardSymbol(item, theme)} ${item.description.replace(/\s+/gu, " ").trim()} · ${item.status} · ${taskDashboardElapsed(item)}`;
			lines.push(truncateToWidth(row, width));
		}
		lines.push("", theme.bold("Processes"));
		if (this.taskSnapshot.processes.length === 0)
			lines.push(theme.muted("  No background processes"));
		for (const item of this.taskSnapshot.processes) {
			const index = items.indexOf(item);
			const pointer = index === this.state.selected ? "›" : " ";
			lines.push(
				truncateToWidth(
		...
		lines.push("", theme.bold("Questions"));
		if (this.taskSnapshot.questions.length === 0)
			lines.push(theme.muted("  No pending questions"));
		for (const item of this.taskSnapshot.questions) {
			const index = items.indexOf(item);
			const pointer = index === this.state.selected ? "›" : " ";
			lines.push(
				truncateToWidth(
					`${pointer} ${taskDashboardSymbol(item, theme)} ${item.description} · ${String(item.questionCount)} question(s) · ${item.status}`,
					width,
				),
			);
		}
	...
	private taskItems(): TaskDashboardItem[] {
		return [
	...
	private renderAgentProcess(width: number, theme: VspiTheme): string[] {
		const history = this.agentConversation?.blocks ?? [];
		const blocks = [
			...visibleAgentHistoryBlocks(history),
			...liveAgentProcessBlocks(this.agentActivity, history),
		];
		const lines: string[] = [];
		for (const block of blocks) {
			const text = compactAgentProcessText(block.text) || "[empty]";
			if (block.kind === "tool" || block.kind === "error") {
				const name = block.toolName ?? "Unknown";
				lines.push(theme.focus(`◇ ${name}`));
				const summary = block.kind === "error" ? `error · ${text}` : text;
				const styled =
					block.kind === "error" || block.presentation === "error"
						? theme.error(summary)
						: block.presentation === "change"
							? theme.success(summary)
							: theme.muted(summary);
				lines.push(
					...wrapTextWithAnsi(styled, Math.max(1, width - 2)).map(
						(line) => `  ${line}`,
					),
				);
			} else if (block.kind === "final") {
				lines.push(theme.bold(theme.focus("◆ Final")));
				lines.push(
					...wrapTextWithAnsi(theme.bold(text), Math.max(1, width - 2)).map(
						(line) => `  ${line}`,
					),
				);
			} else {
				lines.push(theme.focus("● Commentary"));
				lines.push(
					...wrapTextWithAnsi(text, Math.max(1, width - 2)).map(
						(line) => `  ${line}`,
					),
				);
			}
		...
		if (this.agentConversationLoading)
			return [theme.muted("Loading conversation...")];
		if (this.agentConversationError)
			return [theme.error(this.agentConversationError)];
		return [theme.muted("No process captured")];
	}

	private renderAgentColumns(
		runs: readonly AgentRunSnapshot[],
		width: number,
		rows: number,
		theme: VspiTheme,
	): string[] {
		const listWidth = Math.max(28, Math.min(44, Math.floor(width * 0.4)));
		const detailWidth = Math.max(1, width - listWidth - 1);
		const selected = this.selectedAgentRun();
		const visibleRows = Math.max(1, rows - 1);
		if (this.state.selected < this.agentListScroll)
			this.agentListScroll = this.state.selected;
		if (this.state.selected >= this.agentListScroll + visibleRows) {
			this.agentListScroll = this.state.selected - visibleRows + 1;
		}
		this.agentListScroll = Math.max(
			0,
			Math.min(this.agentListScroll, Math.max(0, runs.length - visibleRows)),
		);
		const list = runs
			.slice(this.agentListScroll, this.agentListScroll + visibleRows)
			.map((run, offset) => {
				const index = this.agentListScroll + offset;
				const selectedRow = index === this.state.selected;
				const pointer = selectedRow ? theme.focus("▶") : " ";
				const symbol = agentRunSymbol(run, theme);
				const line = truncateToWidth(
					`${pointer} ${symbol} ${run.codename ?? run.agentId} · ${run.taskTitle ?? run.task}  ${run.status}  ${agentElapsed(run)}`,
					listWidth,
				);
		...
		for (let index = 0; index < visibleRows; index += 1) {
			result.push(
				`${padLine(list[index] ?? "", listWidth)}${theme.muted("│")}${padLine(detail[index] ?? "", detailWidth)}`,
			);
		}
		return result;
	}
	...
	private renderCron(width: number, theme: VspiTheme): string[] {
		if (this.cronTasks.length === 0)
			return [theme.muted(padLine("No scheduled tasks", width))];
		return this.cronTasks.map((task, index) => {
			const status = task.lastError
				? theme.error("failed")
				: theme.focus("scheduled");
			const schedule = task.cron
				? task.cron
				: task.runAt
					? formatCronLocalTime(task.runAt)
					: "—";
	...
	private renderEffort(width: number, theme: VspiTheme): string[] {
		...
		if (this.effortLevels.length === 1) {
			rows.push(theme.muted(padLine("当前模型固定使用此 Effort", width)));
		}
		return rows;
	}
	...
	private renderApproval(width: number, theme: VspiTheme): string[] {
		...
		const inset = (line: string) =>
			padLine(`${" ".repeat(gutter)}${padLine(line, contentWidth)}`, width);
		const target =
			request.action.target ?? request.action.operation ?? request.category;
		const badgeWidth = Math.min(8, contentWidth);
		const policyBadge = theme.policyBadge(
			request.policy,
			centerText(request.policy, badgeWidth),
		);
		...
		for (const command of commandRows) {
			lines.push(
				inset(
					theme.codeBlock(
						centerText(theme.warning(theme.bold(command)), contentWidth),
					),
				),
			);
		}
		...
		if (this.approvalReasonEditing) {
			...
			lines.push(
				inset(
					theme.selected(
						padLine(
							` ${this.approvalReason}${this._cursor(theme)} `,
							contentWidth,
						),
					),
				),
		...
		const choices = [
			...
			...(request.requiredPolicy
				? [
						{
							label: `提升到 ${request.requiredPolicy} 并执行`,
							description: "调整到可执行该操作的等级",
						},
					]
				: []),
		...
		choices.forEach((choice, index) => {
			...
			const labelWidth = Math.min(
				30,
				Math.max(18, Math.floor(optionWidth * 0.45)),
			);
			const row =
				optionWidth >= 52
					? `${padLine(numberedLabel, labelWidth)}${theme.muted(choice.description)}`
					: numberedLabel;
			lines.push(
				inset(
					`${" ".repeat(optionIndent)}${selectedLine(row, index === this.state.selected, optionWidth, theme)}`,
				),
			);
			if (optionWidth < 52)
				lines.push(
					theme.muted(
						inset(`${" ".repeat(optionIndent + 4)}${choice.description}`),
					),
				);
		});
		return lines;
	...
	private renderPlan(
		...
		if (this.workflowSnapshot)
			return this.renderWorkflowPlan(width, theme, focused);
		const items = this.visiblePlanItems();
		if (items.length === 0) return [padLine("", width)];
		const snapshot = this.planSnapshot;
		const leaves = this.planItems.filter((item) => item.group !== true);
		const complete = leaves.filter((item) => item.status === "done").length;
		const lines = snapshot
			? [
					...wrapHeadingWithRight(
						theme.blue(theme.bold(snapshot.title)),
						theme.muted(
							`r${snapshot.revision} · ${complete}/${this.planItems.length}`,
						),
						width,
					),
					...wrapPrefixedLine(
						`${theme.warning(theme.bold("目标"))}  `,
						snapshot.goal,
						width,
					),
				]
			: [
						alignRight(
							theme.blue(theme.bold("Plan")),
							theme.muted(`${complete} / ${leaves.length}`),
							width,
						),
					];
				...
				item.status === "done"
					...
					: item.status === "blocked"
						...
						: item.status === "in_progress"
							? theme.focus("●")
							: theme.muted("○");
			const label = item.focused
				? theme.focus(theme.bold(item.label))
				: item.label;
		...
		if (snapshot) {
			if (this.planActionMenu) {
				lines.push(
					selectedLine("状态", this.planActionIndex === 0, width, theme),
				);
				lines.push(
					selectedLine("焦点", this.planActionIndex === 1, width, theme),
				);
				lines.push(
					selectedLine("下一步", this.planActionIndex === 2, width, theme),
				);
			}
			if (this.planNextActionEditing) {
				lines.push(
					theme.focus(
						padLine(
							`下一步  ${this.planNextActionInput}${theme.inverse(" ")}`,
							width,
						),
					),
				);
	...
	private renderWorkflowPlan(
		...
		if (snapshot.status !== "ready" || !snapshot.delivery) {
			return [
				...wrapHeadingWithRight(
					...
					theme.warning(
						WORKFLOW_STATUS_LABELS[snapshot.status] ?? snapshot.status,
					),
					width,
				),
		...
		const lines = [
			...wrapHeadingWithRight(
				...
				theme.muted(
					`${DELIVERY_STATUS_LABELS[delivery.status] ?? delivery.status} · 修订 ${delivery.revision}`,
				),
				width,
			),
			...wrapTextWithAnsi(
			...
			theme.border(
				padLine((theme.capabilities.unicode ? "─" : "-").repeat(width), width),
			),
		];
		const idWidth = Math.max(
			...delivery.milestones.map((milestone) => milestone.id.length),
		);
		this.visiblePlanItems().forEach((item, index) => {
			const milestone = delivery.milestones.find(
			...
			const statusText =
				milestone?.status === "pending_stone" ? " · 待锚定" : "";
		...
		if (delivery.currentMilestoneId) {
			lines.push(
				...wrapPrefixedLine(
					"当前里程碑  ",
					theme.blue(delivery.currentMilestoneId),
					width,
				),
			);
		}
	...
	private renderPrompt(width: number, theme: VspiTheme): string[] {
		...
		const lines = [
			alignRight(
				...
				snapshot.resolved.disabled
					? theme.warning("Off")
					: theme.muted(
							`${snapshot.resolved.scope}${snapshot.resolved.pinned ? " · pinned" : ""}`,
						),
				width,
			),
		];
		snapshot.profiles.forEach((profile, index) => {
			const active =
				profile.active || profile.id === snapshot.resolved.profileId;
			const evaluation =
		...
		for (const rule of snapshot.rules) {
			lines.push(
				padLine(`规则  ${rule.enabled ? "✓" : "○"} ${rule.label}`, width),
			);
		}
		const labels: Record<
			PromptPanelSnapshot["effectiveSegments"][number]["source"],
			string
		> = {
			"pi-base": "Core base",
			system: "SYSTEM",
		...
		for (const segment of snapshot.effectiveSegments) {
			lines.push(
				padLine(
					`${labels[segment.source]}  ${redactPrompt(segment.content)}`,
					width,
				),
			);
		}
		if (this.promptImportEditing)
			lines.push(
				selectedLine(`导入路径  ${this.promptImportPath}`, true, width, theme),
			);
		return lines;
	}
	...
	private renderQuestion(width: number, theme: VspiTheme): string[] {
		...
		if (this.questionReview) {
			const lines = [
				...
				theme.border(
					padLine(
						(theme.capabilities.unicode ? "─" : "-").repeat(width),
						width,
					),
				),
				"",
			...
			for (const question of this.questions) {
				...
				lines.push(
					question.skipped
						? theme.muted(padLine(`  ${answer}`, width))
						: theme.success(padLine(`  ✓ ${answer}`, width)),
				);
			}
			return lines;
		}
		...
		const lines = [
			...
			inset(
				theme.border(
					(theme.capabilities.unicode ? "─" : "-").repeat(contentWidth),
				),
			),
			"",
		];
		if (this.questionDirectAnswer || question.kind === "freeText") {
			...
			const input = (
				this.questionInput.render(Math.max(1, contentWidth - 2))[0] ?? ""
			).replace(/^> /, "");
		...
		const labelWidth = Math.min(
			24,
			Math.max(...decorated.map(({ label }) => visibleWidth(label))),
		);
		const labelsFitAlignmentColumn = decorated.every(
			({ label }) => visibleWidth(label) <= labelWidth,
		);
		const inlineDescriptions =
			...
			decorated.every(({ option }) => {
				...
				return (
					!description ||
					labelWidth + 2 + visibleWidth(description) <= itemContentWidth
				);
			});
		const itemLine = (
			value: string,
		...
		decorated.forEach(({ option, selected, label }) => {
			...
			if (description && inlineDescriptions) {
				lines.push(
					inset(
						itemLine(
							`${padLine(label, labelWidth + 2)}${theme.muted(description)}`,
							selected,
						),
					),
				);
				...
				if (description) {
					for (const descriptionLine of wrapTextWithAnsi(
						description,
						Math.max(1, itemContentWidth - 2),
					)) {
						lines.push(
							inset(
								itemLine(`  ${theme.muted(descriptionLine)}`, selected, true),
							),
						);
					}
				}
...
function questionAnswerText(question: Question): string {
	...
	if (typeof question.answer === "string")
		return (labels.get(question.answer) ?? question.answer) || "未回答";
	return "未回答";
}
...
function planTreePrefix(
	...
	for (let depth = 0; depth < item.depth - 1; depth += 1) {
		...
		while (ancestorIndex >= 0 && (items[ancestorIndex]?.depth ?? -1) > depth)
			ancestorIndex -= 1;
		prefix +=
			ancestorIndex >= 0 && hasLaterPlanSibling(items, ancestorIndex)
				? "│  "
				: "   ";
	}
	return theme.muted(
