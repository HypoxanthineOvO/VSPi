import { sliceByColumn, stripTerminalSequences } from "@moonshot-ai/pi-tui";
...
const PURE_TAIL_FRAME =
	// biome-ignore lint/suspicious/noControlCharactersInRegex: ANSI control sequences are the protocol being parsed.
	/^\u001b\[\?2026h((?:\u001b\[\d+;\d+H)?\u001b\[\?25[hl])\u001b\[\?2026l$/u;
// biome-ignore lint/suspicious/noControlCharactersInRegex: ANSI control sequences are the protocol being parsed.
...
function parseFrame(data: string, rows: number): ParsedFrame | undefined {
	if (
		!data.includes(BEGIN_SYNC) ||
		!data.endsWith(END_SYNC) ||
		IMAGE_SEQUENCE.test(data)
	)
		return undefined;
...
function simulateShift(
	...
	if (direction === "up") {
		for (let row = start; row < end - amount; row += 1)
			simulated[row] = previous[row + amount] ?? "";
		for (let row = end - amount; row < end; row += 1) simulated[row] = "";
	} else {
		for (let row = end - 1; row >= start + amount; row -= 1)
			simulated[row] = previous[row - amount] ?? "";
...
function findShift(
	...
	const comparablePrevious = previous.map((line) =>
		stripTerminalSequences(sliceByColumn(line, 0, contentWidth, true)),
	);
	const comparableNext = next.map((line) =>
		stripTerminalSequences(sliceByColumn(line, 0, contentWidth, true)),
	);
	...
	const addRuns = (direction: "up" | "down", amount: number) => {
		...
		const flushRun = () => {
			...
			const regionEnd =
				direction === "up"
					? runStart + runLength + amount
					: runStart + runLength;
			const simulated = simulateShift(
				previous,
				rows,
				direction,
				amount,
			...
			const rewrites = next.flatMap((line, row) =>
				line === simulated[row] ? [] : [row],
			);
	...
	return candidates
		...
		.sort(
			(left, right) =>
				left.rewrites.length - right.rewrites.length ||
				right.regionEnd - left.regionEnd,
		)[0];
}
...
function renderShift(
	...
	const rewrites = candidate.rewrites
		.map((row) => `\u001b[${row + 1};1H\u001b[2K${next[row] ?? ""}`)
		.join("");
...
export class TerminalFrameOptimizer {
	...
	constructor(private readonly enableRegionShift = false) {}
	...
	optimize(data: string, rows: number, columns = 80): string {
		...
		if (frame) {
			...
			const previous = hadScreen
				? this.screen
				: Array.from({ length: safeRows }, () => "");
			const next = applyUpdates(previous, frame, safeRows);
			this.screen = next;
			if (
				this.enableRegionShift &&
				hadScreen &&
				!frame.fullRedraw &&
				frame.explicitCursor
			) {
				const candidate = findShift(
					previous,
					next,
					frame.updates.size,
					safeColumns,
