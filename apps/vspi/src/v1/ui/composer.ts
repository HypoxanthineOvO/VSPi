...
import { formatActivityElapsed } from "./activity.js";
...
export class Composer implements Component, Focusable {
  ...
  private workingLabel(activity: ComposerActivity): string {
    ...
    const elapsed = formatActivityElapsed(activity.elapsedSeconds);
    if (activity.style === 2) return `${ball} ${this.theme.bold(activity.label)} ${this.theme.muted(elapsed)}`;
    const cluster = unicode
      ...
      : this.theme.muted("...");
    return `${ball} ${this.theme.bold(activity.label)} ${this.theme.muted(elapsed)}  ${cluster}`;
  }
