...
`/copy` 复制最近一条完整回复。SSH 下通过终端 OSC 52 请求写入本地剪贴板，不需要远端安装 `xclip` 或图形桌面；tmux 会优先尝试其剪贴板转发，再回退到透传。全屏选区复制使用同一策略。终端请求无法确认本地是否接受，若未生效需检查终端权限、OSC 52 支持及 tmux 设置。OSC 52 原文超过 100 KB 时会明确拒绝，不截断复制。

粘贴服务器上可访问的 PNG/JPEG/WebP/GIF 文件路径，会自动添加图片附件。支持绝对/相对路径、`~/`、带引号的路径、转义空格和本地 `file://` URL。文件必须在运行 VSPi 的机器上，不会自动读取或上传 SSH 客户端电脑的文件。导入期间输入会等待，Enter 在附件就绪后才提交；失败保留路径并取消这次待提交的 Enter。Esc 可取消正在导入的图片，切换会话会丢弃旧会话的迟到结果。仅在路径经用户粘贴后读取，不扫描消息中的任意文件引用。

SSH 下收到 Ctrl+V/Alt+V 图片快捷键时，会提示改用服务器图片路径；本地会话继续尝试系统图片剪贴板。服务器路径只是普通文字时，请检查该文件的存在性、权限、支持的图片格式及 20 MiB 大小限制。
...
`/model` 在同一 Provider 内按模型家族排列，再按版本与价格排序。同一协议和地址下，指向相同实际模型 ID 的配置名、目录明确声明的别名会合并为一行；详情的 `Model ID` 列出所有已知标识，搜索任一标识均可找到。例如 `kimi-k3` 与 `k3` 显示为 `Model ID: kimi-k3, k3`，256K 变体单独成组。显示名称相同本身不作为合并依据，跨 Provider、协议或地址也不合并。当前会话已选择的配置名及其参数保持不变；Subagent 候选按家族靠拢，但保留独立配置名供分别管理。
