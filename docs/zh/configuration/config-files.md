...
`loop_control` 控制 Agent 执行循环的步数上限、单步尝试次数上限、重试退避节奏，以及触发上下文自动压缩的阈值。

| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `max_steps_per_turn` | `integer` | — | 单轮最大步数；不设或设为 `0` 则无上限 |
| `max_attempts_per_step` | `integer` | `10` | 单步失败后的最大总尝试次数（含首次尝试） |
| `retry_initial_delay_ms` | `integer` | `500` | 单步重试的初始退避等待（毫秒）；每次重试翻倍，上限为 `retry_max_delay_ms`，并附加最多 25% 的随机抖动。供应商返回 `Retry-After` 时优先使用该值 |
| `retry_max_delay_ms` | `integer` | `32000` | 重试退避等待的上限（毫秒） |
