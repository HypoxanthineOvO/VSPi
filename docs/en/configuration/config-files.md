...
`loop_control` governs the step count limit, the per-step attempt limit, the retry backoff timing, and the threshold that triggers automatic context compaction in the Agent execution loop.
...
| `retry_initial_delay_ms` | `integer` | `500` | Initial backoff wait before the first retry of a failing step, in milliseconds; the wait doubles per attempt up to `retry_max_delay_ms`, with up to 25% random jitter. A provider-supplied `Retry-After` value always takes priority |
| `retry_max_delay_ms` | `integer` | `32000` | Upper bound of the retry backoff wait, in milliseconds |
