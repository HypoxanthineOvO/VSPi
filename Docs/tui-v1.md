...
同服务器的 Session 使用独占 lease、heartbeat 和 `0600` Unix control socket 保证单写者与单一可交互前台。Sessions 面板将其他 owner 标为“使用中”，并提供前台迁移、安全分支和取消。迁移不得序列化或重建 Pi 的 live JavaScript 工具栈：旧进程保留真实 Agent 执行栈，新 TUI 在 lease acquisition 完成前先启动为等待态，只允许处理 socket 转发的 Question/Approval；旧 TUI 从接受请求起锁定全部输入。已经 pending 或迁移后出现的 Question/Approval 都在新 TUI 弹出，回答回传并解析到旧进程原 Promise。旧进程必须等 generation、tool、compaction 与原生队列全部 idle，且不得调用 Agent abort；落盘后显示已移交并退出。新进程随后获得 lease、重新打开最终 Session 并恢复完整 Transcript、工具状态和普通输入。多个等待进程中只有被接受者可以回答；接管方在安全点前断开时旧 TUI 解除移交锁并恢复 pending 交互。占用中分支只能复制最后一个完整 assistant 回复及其之前的稳定历史。普通退出不保留后台任务；resume 只恢复落盘内容，最后一条为 user 或 aborted assistant 时显示中断标记，不自动重试。

Linux lease schema v2 额外绑定 boot ID、`/proc/<pid>/stat` start time 和 UID。owner 在握手前无响应，或接受后 heartbeat 停止 15 秒时，只有用户显式确认并验证同一进程实例退出后才能恢复：先 SIGTERM，超时后再次确认 SIGKILL。recovery claim 串行多个恢复者；旧 schema、异地主机、不同 UID 与不可验证进程均 fail closed。该机制只承诺同服务器，跨服务器共享存储留待后续版本。
